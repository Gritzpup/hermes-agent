...
export const councilSources = ['pi', 'api', 'cli', 'rules'] as const;
...
export function getCouncilSourceSummary(decision: CouncilDecisionLike): CouncilSourceSummary {
  ...
  if (sources.every((source) => source === 'pi')) {
    return { label: 'Pi only', tone: 'healthy', fallback: false, sources };
  }
  ...
  const transportSources = sources.filter((source) => source === 'pi' || source === 'cli');
