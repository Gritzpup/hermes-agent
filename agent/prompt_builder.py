...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        # Compact index: names only, grouped by category. Per-skill descriptions
        # are dropped from the system prompt (saves ~1.2k tokens at startup);
        # the model uses skill_view(name) to read full content on demand.
        # Category descriptions retained as routing hints.
        index_lines = []
        ...
        result = (
            "## Skills (mandatory)\n"
            "Scan the index below. If any skill is even partially relevant, "
            "load it with skill_view(name) before acting — skills encode the "
            "user's preferred commands, workflows, and conventions. "
            "For Hermes-config tasks (CLI, models, providers, tools, plugins), "
            "load `hermes-agent` first. "
            "Use skill_manage(action='patch') to fix bad skills; offer to save "
            "new skills after iterative tasks.\n"
            "\n"
            "<available_skills>\n"
            + "\n".join(index_lines) + "\n"
            "</available_skills>\n"
            "\n"
            "Only proceed without loading a skill if none apply."
        )
