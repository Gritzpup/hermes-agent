 ...
 */
export function buildDirectorPrompt(ctx, regime, playbookApplications) {
    const playbookSummary = playbookApplications.length > 0
        ? `PLAYBOOK SWITCHES ALREADY APPLIED THIS CYCLE:\n${playbookApplications.map((p) => `  - ${p.agentId}: switched to template '${p.templateName}' (${p.regime}) — ${p.reason}`).join('\n')}`
    ...
    const lines = [
        ...
        JSON.stringify((ctx.market?.compositeSignal ?? [])
            .slice(0, 8)
            .map((s) => ({
            symbol: s.symbol,
        ...
        '',
        'LOSS CLUSTERS (top 3):',
        JSON.stringify(ctx.lossClusters?.lossClusters ? ctx.lossClusters.lossClusters.slice(0, 3) : [], null, 2),
        '',
