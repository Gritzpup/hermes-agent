...
function matchesFilter(event: ReplayEventRecord, filter: ReplayFilter): boolean {
  if (filter.type && event.type !== filter.type) return false;
  if (!filter.strategyId) return true;
  if (event.strategyId === filter.strategyId || event.agentId === filter.strategyId) return true;
  if (typeof event.strategy === 'string' && event.strategy === filter.strategyId) return true;
  return false;
}

export class ReplayEngine {
  getTimeline(limit = 200, filter: ReplayFilter = {}): ReplayEventRecord[] {
    const events = readJsonLines<ReplayEventRecord>(EVENT_LOG_PATH).filter((event) => matchesFilter(event, filter));
    return events.slice(-limit);
  }

  getReconstruction(limit = 1_000): Record<string, unknown> {
    const events = readJsonLines<ReplayEventRecord>(EVENT_LOG_PATH).slice(-limit);
    const journal = readJsonLines<TradeJournalEntry>(JOURNAL_LEDGER_PATH).slice(-limit);
    const eventCounts = new Map<string, number>();
    const agentStates = new Map<string, Record<string, unknown>>();
    const strategyStates = new Map<string, Record<string, unknown>>();
    const journalSummaries = new Map<string, {
      strategyId: string;
      strategy: string;
      lane: string;
      trades: number;
    ...
    const strategyNameLookup = new Map<string, { strategyId: string; lane: string }>();
    const configTransitions: ReplayEventRecord[] = [];
    const allocationTransitions: ReplayEventRecord[] = [];
    const laneLearningTransitions: ReplayEventRecord[] = [];
    let lastTick: ReplayEventRecord | null = null;
    ...
    for (const event of events) {
      ...
      if (type === 'tick') {
        lastTick = event;
        const agents = Array.isArray(event.agents) ? event.agents as Array<Record<string, unknown>> : [];
...
let replayEngine: ReplayEngine | undefined;
