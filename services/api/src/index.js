...
const learningLoop = new LearningLoop(() => paperEngine.getSnapshot().agents, (agentId, config) => paperEngine.applyAgentConfig(agentId, config));
...
app.get('/api/insider-radar', (_req, res) => {
    try {
        ...
        res.json({ signals: [], filings: [], lastPollAt: null });
    }
});
...
app.get('/api/historical-context', (_req, res) => {
    try {
        ...
        res.json({ macro: {}, fearGreedHistory: [], regimeChanges: [] });
    }
});
...
app.get('/api/quarter-outlook', async (_req, res) => {
    ...
    try {
        ...
        res.json({ regime: 'unknown', posture: 'defensive', reasoning: 'Unavailable.' });
    }
});
app.get('/api/capital-allocation', (_req, res) => {
    try {
        ...
        res.json({ capital: 0, sleeves: [], notes: ['Error building allocation snapshot.'] });
    }
});
...
async function triggerNightlyTraining() {
    try {
...
function scheduleLogRotation() {
    ...
    if (!fs.existsSync(PAPER_LEDGER_DIR))
        fs.mkdirSync(PAPER_LEDGER_DIR, { recursive: true });
    // Run once at startup
...
scheduleLogRotation();
function computeJournalAggregates(entries) {
...
function buildBucket(entries, getKey) {
    const groups = new Map();
    for (const e of entries)
        groups.set(getKey(e), [...(groups.get(getKey(e)) ?? []), e]);
    const buckets = [];
...
app.get('/api/emergency-halt', (_req, res) => {
    if (fs.existsSync(EMERGENCY_HALT_FILE)) {
        try {
            res.json({ active: true, ...JSON.parse(fs.readFileSync(EMERGENCY_HALT_FILE, 'utf8')) });
        }
        catch {
            res.json({ active: true, error: 'parse error' });
        }
    }
    else {
