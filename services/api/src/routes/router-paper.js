...
import { BROKER_ROUTER_URL, BROKER_STARTING_EQUITY, MARKET_DATA_URL, LIVE_BROKER, } from '../lib/constants.js';
import { fetchJson } from '../lib/utils-http.js';
import { round } from '../lib/utils-generic.js';
import { normalizeBrokerAccounts, normalizeBrokerPositions, dedupeMarketSnapshots, extractBrokerPnL } from '../lib/utils-normalization.js';
    ...
    setImmediate(async () => {
        try {
            const snap = await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error('snapshot-timeout')), 5000))
            ]);
            if (snap) {
...
export function createPaperRouter(deps) {
    ...
    setTimeout(() => {
        try {
            ...
            console.warn('[paper-desk] cache warm failed:', e.message);
        }
    }, 2000);
    ...
    router.get('/paper-desk', async (_req, res) => {
        ...
        if (_paperDeskCache.data) {
            ...
            try {
                ...
                if (brokerState) {
                    ...
                    paperDesk.liveBrokerEquity = Math.round(brokerAccts.reduce((s, a) => s + (a.equity ?? 0), 0) * 100) / 100;
                    // COO FIX: Add Coinbase simulated equity ($100K + journal PnL) to total.
                    const cbRollup = (paperDesk.brokerRollups ?? []).find((r) => r.broker === 'coinbase-live');
                    ...
                    if (paperDesk.analytics)
                        paperDesk.analytics.totalOpenRisk = round(totalOpenRisk, 2);
                }
            }
        ...
        try {
            ...
            if (brokerState) {
                ...
                const lanes = paperDesk.lanes ?? [];
                const firmTrades = lanes.reduce((s, l) => s + (l.trades ?? 0), 0);
                const firmWins = lanes.reduce((s, l) => s + (l.wins ?? 0), 0);
                const firmLosses = lanes.reduce((s, l) => s + (l.losses ?? 0), 0);
                const firmRealized = Math.round(lanes.reduce((s, l) => s + (l.realizedPnl ?? 0), 0) * 100) / 100;
