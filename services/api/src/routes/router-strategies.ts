...
import { getAmplification } from '../coo-gates.js';
...
export function createStrategyRouter(deps: StrategyRouterDeps) {
  ...
  router.get('/pairs', (_req, res) => {
    const btcSnap = deps.paperEngine.getMarketSnapshots().find((s: any) => s.symbol === 'BTC-USD');
    const ethSnap = deps.paperEngine.getMarketSnapshots().find((s: any) => s.symbol === 'ETH-USD');
    const controls = deps.marketFeed.getLaneControls();
    res.json({
      control: controls.find((c: any) => c.strategyId === 'pairs-btc-eth') ?? null,
  ...
  router.get('/maker', (_req, res) => {
    ...
    res.json({
      controls: [
        controls.find((c: any) => c.strategyId === 'maker-btc-usd') ?? null,
        controls.find((c: any) => c.strategyId === 'maker-eth-usd') ?? null
      ].filter(Boolean),
...
function buildStrategySnapshots(deps: StrategyRouterDeps): any[] {
  const paperDesk = deps.paperEngine.getSnapshot();
  const readiness = deps.paperEngine.getLiveReadiness();
  const controls = deps.marketFeed.getLaneControls();

  const scalpers = readiness.agents.map((agent: any) => {
    const paperAgent = paperDesk.agents.find((candidate: any) => candidate.id === agent.agentId);
    const failedGates = agent.gates.filter((gate: any) => !gate.passed);
    return {
      id: agent.agentId,
      ...
      summary: failedGates.map((gate: any) => `${gate.name}: ${gate.actual}`).join(' | ') || 'Cleared current readiness gates.'
    };
  });

  const btcPrice = deps.paperEngine.getMarketSnapshots().find((s: any) => s.symbol === 'BTC-USD')?.lastPrice ?? 0;
  const ethPrice = deps.paperEngine.getMarketSnapshots().find((s: any) => s.symbol === 'ETH-USD')?.lastPrice ?? 0;
  
  const pairsState = deps.pairsEngine.getState(btcPrice, ethPrice);
  const pairsStats = deps.pairsEngine.getStats();
  const pairsControl = controls.find((c: any) => c.strategyId === 'pairs-btc-eth');
  const pairsSnapshot = {
    id: 'pairs-btc-eth',
  ...
  const makerSnapshots = deps.makerEngine.getSnapshot().states.map((state: any) => {
    const control = controls.find((c: any) => c.strategyId === `maker-${state.symbol.toLowerCase()}`);
    return {
...
function buildResearchCandidates(snapshots: MarketSnapshot[]): any[] {
  return snapshots
