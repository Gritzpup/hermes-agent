...
export class LearningLoop {
    ...
    getPerformance;
    applyConfig;
    lastEvolutionByAgent = new Map();
    constructor(getPerformance, applyConfig) {
    ...
    async review() {
        if (this.evolving || this.reviewInFlight)
            return;
    ...
    async evolveAgent(agent) {
        ...
        try {
            ...
            if (bestResult && bestConfig && bestResult.profitFactor > currentPF * 1.15 && bestResult.profitFactor > 1.0) {
                ...
                if (safeConfig.sizeFraction < 0.01)
                    safeConfig.sizeFraction = 0.01;
                if (safeConfig.sizeFraction > 0.15)
                    safeConfig.sizeFraction = 0.15;
                if (safeConfig.stopBps < 3)
                    safeConfig.stopBps = 3;
                if (safeConfig.targetBps < 3)
                    safeConfig.targetBps = 3;
                if (safeConfig.maxHoldTicks < 5)
                    safeConfig.maxHoldTicks = 5;
    ...
    generateCandidates(agent) {
        const base = agent.currentConfig;
        const candidates = [];
        const styles = ['momentum', 'mean-reversion', 'breakout'];
    ...
    async runBacktest(config, symbol, startDate, endDate) {
        const agentConfig = {
            style: config.style,
