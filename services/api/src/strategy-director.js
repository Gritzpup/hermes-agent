...
export class StrategyDirector {
    ...
    start() {
        ...
        subscriber.subscribe(TOPICS.RISK_SIGNAL, (err) => {
            if (err)
                logger.error({ err }, 'Failed to subscribe to Risk Signals');
        });
        const emergencySignals = new Set(['volatility-spike', 'risk-off', 'correlation-break']);
        subscriber.on('message', (channel, message) => {
            if (channel !== TOPICS.RISK_SIGNAL)
                return;
            try {
                const signal = JSON.parse(message);
                if (!emergencySignals.has(signal.type))
                    return;
                const now = Date.now();
                const cooldownMs = 300_000; // 5-min cooldown between emergency cycles
                if (now - this.lastEmergencyCycleAt < cooldownMs)
                    return;
    ...
    async runCycle() {
        ...
        try {
            ...
            if (regime !== this.lastDetectedRegime && this.lastDetectedRegime !== 'unknown') {
                ...
                historicalCtx.recordRegimeChange(regime, this.lastDetectedRegime, `Strategy Director cycle ${runId.slice(0, 8)}`, fngTrend?.current ?? null, context.market?.btcPrice ?? null);
            }
    ...
    async gatherContext() {
        ...
        const [brokerResp, clusterResp, simResp] = await Promise.allSettled([
            ...
            (async () => {
                ...
                clearTimeout(timeout);
                if (!resp.ok)
                    throw new Error('clusters non-ok');
                return await resp.json();
            })(),
            (async () => {
                const controller = new AbortController();
        ...
        return {
            ...
            configs: configs.slice(0, 20).map((c) => ({
                ...
                targetBps: c.config.targetBps,
                stopBps: c.config.stopBps,
                maxHoldTicks: c.config.maxHoldTicks,
                sizeFraction: c.config.sizeFraction,
            })),
            recentJournal: journal.slice(0, 10).map((j) => ({
            ...
            news: {
                macroSignal: newsSnapshot.macroSignal,
                articles: (newsSnapshot.articles ?? []).slice(0, 10),
                insiderSignals: insiderSnapshot.signals,
            },
            market: marketSnapshot,
            brokerAccounts: brokerData,
            lossClusters: clusters,
            forwardSimulation: simulations ? {
                overall: simulations.overall,
                classSummaries: (simulations.classSummaries ?? []).slice(0, 5)
            } : null,
            asOf: new Date().toISOString()
        };
    }
    buildPrompt(ctx, regime, playbookApplications) {
     ...
     */
    async evaluateWithFallback(prompt) {
        ...
        if (process.env.STRATEGY_DIRECTOR_SKIP_CLAUDE !== '1') {
            try {
                const { stdout } = await runProcess(CLAUDE_BIN, ['-p', '--output-format', 'json', '--model', CLAUDE_MODEL], { cwd: WORKSPACE_ROOT, timeoutMs: STRATEGY_DIRECTOR_TIMEOUT_MS, stdin: prompt });
        ...
        try {
            const { stdout } = await runProcess(GEMINI_BIN, ['-m', GEMINI_MODEL, '--output-format', 'json', '-p', '-'], { cwd: WORKSPACE_ROOT, timeoutMs: STRATEGY_DIRECTOR_TIMEOUT_MS, stdin: prompt });
            const envelope = JSON.parse(stdout);
    ...
    async applyDirective(parsed, runId, startedAt, regime, playbookApplications) {
    ...
    async runForwardSimulation(proposed, context) {
        ...
        for (const agentConfig of engine.getAgentConfigs()) {
            currentConfigMap.set(agentConfig.agentId, agentConfig.config);
        }
        try {
            ...
            if (!resp.ok)
                return null;
            const outlook = await resp.json();
            const overall = outlook.overall;
            if (!overall)
                return null;
            const lastQ = overall.lastQuarter;
            const nextQ = overall.nextQuarter;
            if (!lastQ || !nextQ)
                return null;
            ...
            const hasDefensiveChanges = adjustments.some((a) => {
    ...
    loadLog() {
        try {
            ...
            this.directives = lines.map((line) => JSON.parse(line)).reverse().slice(0, 200);
