...
export class PairsXauBtcEngine {
    ...
    crossBrokerEnabled = process.env.CROSS_BROKER_ENABLED === '1';
    paperMode = process.env.LIVE_TRADING !== '1';
    constructor(startingEquity, journalLedgerPath) {
    ...
    update(xauPrice, btcPrice) {
        if (xauPrice <= 0 || btcPrice <= 0)
            return;
        this.tick += 1;
    ...
    openPosition(direction, xauPrice, btcPrice, ratio, spread, zScore, beta, corr) {
    ...
    closePosition(xauPrice, btcPrice, exitSpread, exitZScore, corr, reason) {
        if (!this.position)
            return;
    ...
    getUnrealizedPnl(xauPrice, btcPrice) {
        if (!this.position)
            return 0;
        const pos = this.position;
        let xauPnl;
    ...
    getEquity(xauPrice, btcPrice) {
        return round(this.cash + (this.position
            ? this.position.legNotional * 2 + this.getUnrealizedPnl(xauPrice, btcPrice)
            : 0), 2);
    }
}
function computeLogBeta(xauSeries, btcSeries) {
...
function computeReturnCorrelation(xauSeries, btcSeries) {
    if (xauSeries.length < 3 || btcSeries.length < 3)
        return 0;
    const xauReturns = [];
    const btcReturns = [];
