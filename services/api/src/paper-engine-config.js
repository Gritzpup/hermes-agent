export function buildAgentConfigs(realPaperAutopilot) {
    return [
...
export function getDefaultAgentConfig(agentId, realPaperAutopilot) {
    return buildAgentConfigs(realPaperAutopilot).find((config) => config.id === agentId) ?? null;
}
export function withAgentConfigDefaults(config) {
    return {
        ...config,
        broker: config.broker ?? defaultBrokerForSymbol(config.symbol),
        executionMode: config.status === 'contender' ? 'watch-only' : (config.executionMode ?? defaultExecutionModeForSymbol(config.symbol)),
        autonomyEnabled: config.autonomyEnabled ?? defaultAutonomyEnabled(config.symbol)
    };
}
export function defaultBrokerForSymbol(symbol) {
...
export function defaultExecutionModeForSymbol(symbol) {
    if (symbol === 'ETH-USD' || symbol === 'QQQ' || symbol === 'NVDA') {
        return 'broker-paper';
    }
    return 'watch-only';
}
export function defaultAutonomyEnabled(symbol) {
    // @ts-ignore
...
export function validateAgentConfigs(configs) {
    const errors = [];
    const ids = new Set();
    for (const c of configs) {
        ...
        if (ids.has(c.id))
            errors.push(`Duplicate agent id: ${c.id}`);
        ids.add(c.id);
        if (!c.symbol)
            errors.push(`${c.id}: missing symbol`);
        if (!c.name)
            errors.push(`${c.id}: missing name`);
        if (c.targetBps <= 0)
            errors.push(`${c.id}: targetBps must be > 0 (got ${c.targetBps})`);
        if (c.stopBps <= 0)
            errors.push(`${c.id}: stopBps must be > 0 (got ${c.stopBps})`);
        if (c.sizeFraction < 0 || c.sizeFraction > 1)
            errors.push(`${c.id}: sizeFraction must be 0-1 (got ${c.sizeFraction})`);
        if (c.maxHoldTicks <= 0)
            errors.push(`${c.id}: maxHoldTicks must be > 0 (got ${c.maxHoldTicks})`);
        if (c.spreadLimitBps < 0)
            errors.push(`${c.id}: spreadLimitBps must be >= 0 (got ${c.spreadLimitBps})`);
        if (!['momentum', 'mean-reversion', 'breakout', 'arbitrage'].includes(c.style))
            errors.push(`${c.id}: invalid style '${c.style}'`);
        if (!['coinbase-live', 'oanda-rest', 'alpaca-paper'].includes(c.broker))
            errors.push(`${c.id}: invalid broker '${c.broker}'`);
    }
    if (errors.length > 0) {
