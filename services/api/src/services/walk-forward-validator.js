...
function evaluateRegimeGate(regimeMetrics) {
    for (const rm of regimeMetrics) {
        if (rm.trades < MIN_TRADES_PER_REGIME)
            continue;
        if (rm.expectancy < -2) {
 ...
 */
export async function runValidationFold(challengerConfig, symbol, fold, timeoutMs = 120_000) {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), timeoutMs);
    try {
        const result = await runBacktest(challengerConfig, symbol, fold.valStart, fold.valEnd, controller.signal);
...
export function summarizeValidation(foldResults) {
