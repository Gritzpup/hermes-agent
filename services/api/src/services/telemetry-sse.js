import { BROKER_ROUTER_URL, BROKER_STARTING_EQUITY } from '../lib/constants.js';
import { fetchJson, getServiceHealthSnapshot } from '../lib/utils-http.js';
import { normalizeBrokerAccounts, normalizeBrokerPositions, dedupePositions } from '../lib/utils-normalization.js';
...
export class TelemetrySSEService {
    ...
    startSlowLoop() {
        const tick = async () => {
            try {
                ...
                if (brokerState)
                    this.sharedBrokerCache = brokerState;
    ...
    emitTo(res) {
        try {
            ...
            const journalRows = this.deps.paperEngine._journalCache?.rows ?? [];
            const firmLaneRollups = computeLaneRollups(journalRows);
            ...
            const cbPaperAgents = paperDesk.agents.filter((a) => a.broker === 'coinbase-live');
            const cbPaperPnl = cbPaperAgents.reduce((s, a) => s + a.realizedPnl, 0);
            ...
            const payload = {
                ...
                marketIntel: {
                    ...
                    compositeSignals: intelSnapshot.compositeSignal.map((sig) => {
                        const tape = (paperDesk.marketTape ?? []).find((t) => t.symbol === sig.symbol);
                        return {
                            ...sig,
