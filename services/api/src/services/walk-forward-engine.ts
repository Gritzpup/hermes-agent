...
import type { BacktestAgentConfig, BacktestCandle } from '@hermes/contracts';
...
export interface WFRunStatus {
  status: 'idle' | 'running' | 'complete' | 'error';
  startedAt: string | null;
  completedAt: string | null;
  foldsRun: number;
  foldsTotal: number;
  summary: ReturnType<typeof summarizeValidation> | null;
  bestParams: ReturnType<typeof loadBestParams>;
  error: string | null;
}

let wfStatus: WFRunStatus = {
  status: 'idle',
  startedAt: null,
  completedAt: null,
  foldsRun: 0,
...
export function getWalkForwardStatus(): WFRunStatus {
  return { ...wfStatus, bestParams: loadBestParams() };
}

async function fetchCandles(symbol: string, startDate: string, endDate: string): Promise<BacktestCandle[]> {
  ...
  const data = await response.json() as { candles: BacktestCandle[] };
  return data.candles ?? [];
}

export async function runWalkForwardCycle(
  ...
  const start = startDate ?? (() => {
    const d = new Date(end); d.setDate(d.getDate() - 90); return d.toISOString().split('T')[0]!;
  })();
  ...
  try {
    ...
    for (const fw of folds) {
      ...
      let trainResult;
      try {
        trainResult = await runTrainingFold(
          challengerConfig, WF_SYMBOL,
          { fold: fw.foldIndex, trainStart: fw.trainStart, trainEnd: fw.trainEnd }
        );
      } catch (err) {
      ...
      let valResult;
      try {
        valResult = await runValidationFold(
          challengerConfig, WF_SYMBOL,
          {
            fold: fw.foldIndex,
    ...
    if (beatsChampion && passedEnough) {
      const best = {
        agentConfig: challengerConfig as unknown as Record<string, unknown>,
