...
export function loadBestParams(): WFBestParams | null {
  try {
    ensureDir();
    if (!fs.existsSync(BEST_PARAMS_FILE)) return null;
    const raw = fs.readFileSync(BEST_PARAMS_FILE, 'utf8');
    const parsed = JSON.parse(raw);
    if (!parsed || typeof parsed.profitFactor !== 'number') return null;
    return parsed as WFBestParams;
  } catch {
    return null;
  }
}

export function saveBestParams(params: WFBestParams): void {
  ensureDir();
  fs.writeFileSync(BEST_PARAMS_FILE, JSON.stringify(params, null, 2));
}

export function loadFoldResults(): WFFoldResult[] {
  try {
    ensureDir();
    if (!fs.existsSync(FOLD_RESULTS_FILE)) return [];
    const raw = fs.readFileSync(FOLD_RESULTS_FILE, 'utf8');
    const parsed = JSON.parse(raw);
    if (!Array.isArray(parsed)) return [];
    return parsed as WFFoldResult[];
  } catch {
    return [];
  }
}

export function appendFoldResult(result: WFFoldResult): void {
...
export function clearWalkForwardState(): void {
  try {
    if (fs.existsSync(BEST_PARAMS_FILE)) fs.unlinkSync(BEST_PARAMS_FILE);
    if (fs.existsSync(FOLD_RESULTS_FILE)) fs.unlinkSync(FOLD_RESULTS_FILE);
  } catch {
