...
const ASSET_CLASS_CROWDING_CAP = {
    crypto: 35,
    equity: 40,
    forex: 15,
    bond: 20,
...
function getSymbolPolicyMultiplier(symbols) {
    let multiplier = 1.0;
    const notes = [];
...
function strategyWeightCap(kind) {
    switch (kind) {
...
function strategyStatusLabel(status) {
    if (status === 'blocked')
        return 'blocked';
    if (status === 'warming' || status === 'active')
        return 'paper';
    return 'staged';
}
function pickBestOpportunity(opportunities, assetClass) {
...
function pickBestReadiness(readiness, assetClass) {
...
function evaluateStrategyLaneKpiRatio(best) {
    if (!best) {
        return 0;
    }
...
function evaluateCopyKpiRatio(backtest) {
    if (!backtest) {
        return 0;
    }
...
function evaluateMacroKpiRatio(snapshot, backtest) {
    if (!backtest) {
...
function buildScalpingSleeve(context, assetClass) {
    ...
    const liveEligible = Boolean(opportunity
        && readiness
        && opportunity.enabled
...
function buildStrategySleeve(context, kind) {
...
function normalizeTargets(sleeves, deployablePct) {
    ...
    for (const [cls, total] of byClass.entries()) {
        ...
        if (total > cap) {
            ...
            for (const entry of sleeves) {
                if (entry.allocation.assetClass !== cls)
                    continue;
...
export function buildCapitalAllocatorSnapshot(context) {
    const sleeves = [
