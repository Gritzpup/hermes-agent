...
export type KpiGateScope = 'symbol' | 'asset' | 'agent' | 'sleeve' | 'desk';

export interface KpiGateThresholds {
...
export interface KpiGateEvaluation {
  scope: KpiGateScope;
  passed: boolean;
  ratioPct: number;
  grade: 'A' | 'B' | 'C' | 'D' | 'F';
  ...
  warnings: string[];
  summary: string;
  thresholds: KpiGateThresholds;
}

const KPI_THRESHOLDS: Record<KpiGateScope, KpiGateThresholds> = {
  symbol: {
    minSampleCount: 8,
...
function scopeLabel(scope: KpiGateScope): string {
  switch (scope) {
...
export function evaluateKpiGate(input: KpiGateInput): KpiGateEvaluation {
  ...
  const grade: KpiGateEvaluation['grade'] = ratioPct >= 85 ? 'A' : ratioPct >= 75 ? 'B' : ratioPct >= 65 ? 'C' : ratioPct >= 55 ? 'D' : 'F';
