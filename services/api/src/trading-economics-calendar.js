...
function resolveAffectedSymbols(event, country) {
    ...
    if (/\b(gdp|gross domestic)\b/.test(title)) {
        if (c === 'UNITED STATES')
            return ['DXY', 'BTC-USD', 'SPY', 'QQQ'];
        if (c === 'EURO AREA')
            return ['EURUSD', 'BTC-USD', 'SPY'];
        if (c === 'CHINA')
            return ['CNYUSD', 'BTC-USD', 'SPY', 'QQQ'];
        if (c === 'UNITED KINGDOM')
            return ['GBPUSD', 'BTC-USD', 'SPY'];
        if (c === 'JAPAN')
            return ['JPYUSD', 'BTC-USD', 'SPY'];
        return ['BTC-USD', 'SPY'];
    }
 ...
 */
export async function fetchUpcomingMacroEvents() {
 ...
 */
export function checkMacroEmbargo(symbol, macroEvents, nowMs = Date.now()) {
