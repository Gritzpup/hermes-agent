 ...
 */
import { computeRSI, computeRSI2, computeMACD, computeBollinger, computeVwap, computeStochastic, computeWeightedOBI, } from './market-intel-indicators.js';
function round(v, d) { return Number(v.toFixed(d)); }
/** Detect nearest support/resistance levels from recent price action */
export function getSupportResistance(priceHistory, symbol) {
 ...
 */
export function computeCompositeSignal(symbol, orderFlow, fearGreed, priceHistory, volumeHistory, barHistory, fundingRates, hlData, venueDivergence, stableRegime) {
    const reasons = [];
    ...
    if (flow) {
        if (flow.direction === 'buy' && flow.strength === 'strong') {
            score += 40;
            reasons.push(`Strong buy flow (${flow.imbalancePct}% imbalance)`);
        }
        else if (flow.direction === 'buy' && flow.strength === 'moderate') {
            score += 25;
            reasons.push(`Moderate buy flow (${flow.imbalancePct}%)`);
        }
        else if (flow.direction === 'sell' && flow.strength === 'strong') {
            score -= 40;
            reasons.push(`Strong sell flow (${flow.imbalancePct}%)`);
        }
        else if (flow.direction === 'sell' && flow.strength === 'moderate') {
            score -= 25;
            reasons.push(`Moderate sell flow (${flow.imbalancePct}%)`);
        }
    ...
    if (fearGreed) {
        if (fearGreed.contrarian === 'buy') {
            score += 15;
            reasons.push(`Extreme Fear (${fearGreed.value}) = contrarian buy`);
        }
        else if (fearGreed.contrarian === 'sell') {
            score -= 15;
            reasons.push(`Extreme Greed (${fearGreed.value}) = contrarian sell`);
        }
    }
    // Bollinger (weight: 20%)
    const bb = computeBollinger(priceHistory, symbol);
    if (bb) {
        if (bb.pricePosition < 0.1) {
            score += 20;
            reasons.push(`Price at lower Bollinger band (oversold)`);
        }
        else if (bb.pricePosition > 0.9) {
            score -= 20;
            reasons.push(`Price at upper Bollinger band (overbought)`);
        }
        if (bb.squeeze) {
            reasons.push('Bollinger squeeze detected — big move imminent');
        }
    }
    // VWAP (weight: 15%)
    ...
    if (vw) {
        if (vw.signal === 'buy') {
            score += 15;
            reasons.push(`Below VWAP (${vw.deviation}% deviation)`);
        }
        else if (vw.signal === 'sell') {
            score -= 15;
            reasons.push(`Above VWAP (${vw.deviation}% deviation)`);
        }
    }
    // Trend (weight: 10%)
    const prices = priceHistory.get(symbol);
    if (prices && prices.length >= 50) {
        ...
        if (sma20 > sma50) {
            score += 10;
            reasons.push('Short-term trend bullish (SMA20 > SMA50)');
        }
        else {
            score -= 10;
            reasons.push('Short-term trend bearish (SMA20 < SMA50)');
        }
    }
    ...
    if (prices && prices.length >= 15) {
        ...
        if (rsi !== null && rsi !== 100 && rsi !== 0) {
            if (rsi > 70) {
                ...
                score += 5;
                reasons.push(`RSI bullish momentum (${rsi.toFixed(0)})`);
            }
            else if (rsi < 45) {
                score -= 5;
                reasons.push(`RSI bearish momentum (${rsi.toFixed(0)})`);
            }
        }
    }
    ...
    if (prices && prices.length >= 26) {
        ...
        if (macd) {
            if (macd.histogram > 0 && macd.histogramPrev <= 0) {
                score += 10;
                reasons.push('MACD bullish crossover');
            }
            else if (macd.histogram < 0 && macd.histogramPrev >= 0) {
                score -= 10;
    ...
    if (sr) {
        if (sr.nearSupport) {
            score += 10;
            reasons.push(`Near support at ${sr.support}`);
        }
        if (sr.nearResistance) {
            score -= 10;
            reasons.push(`Near resistance at ${sr.resistance}`);
        }
    }
    ...
    if (rsi2 !== null && rsi2 !== 100 && rsi2 !== 0 && hasVolatility) {
        if (rsi2 < 5) {
            score += 35;
            reasons.push(`RSI(2) extreme oversold (${rsi2.toFixed(1)}) — high-prob bounce`);
        }
        else if (rsi2 < 15) {
            score += 20;
            reasons.push(`RSI(2) very oversold (${rsi2.toFixed(1)})`);
        }
        else if (rsi2 < 25) {
            score += 10;
            reasons.push(`RSI(2) oversold (${rsi2.toFixed(1)})`);
        }
        else if (rsi2 > 95) {
            score -= 35;
            reasons.push(`RSI(2) extreme overbought (${rsi2.toFixed(1)}) — high-prob pullback`);
        }
        else if (rsi2 > 85) {
            score -= 20;
            reasons.push(`RSI(2) very overbought (${rsi2.toFixed(1)})`);
        }
        else if (rsi2 > 75) {
            score -= 10;
            reasons.push(`RSI(2) overbought (${rsi2.toFixed(1)})`);
        }
    }
    // Stochastic(14,3,3) confirmation (weight: 10%) — crossover signals
    const stoch = computeStochastic(barHistory, priceHistory, symbol);
    if (stoch) {
        if (stoch.crossover === 'bullish' && stoch.k < 30) {
            score += 10;
            reasons.push(`Stochastic bullish crossover in oversold zone (K=${stoch.k})`);
        }
        else if (stoch.crossover === 'bearish' && stoch.k > 70) {
            score -= 10;
            reasons.push(`Stochastic bearish crossover in overbought zone (K=${stoch.k})`);
        }
        else if (stoch.crossover === 'bullish') {
            score += 4;
            reasons.push(`Stochastic bullish crossover (K=${stoch.k})`);
        }
        else if (stoch.crossover === 'bearish') {
            score -= 4;
            reasons.push(`Stochastic bearish crossover (K=${stoch.k})`);
        }
    }
    ...
    if (stableRegime && (symbol.endsWith('-USD') || symbol.startsWith('BTC') || symbol.startsWith('ETH') || symbol.startsWith('SOL') || symbol.startsWith('XRP'))) {
        if (stableRegime.regime === 'inflow') {
            score += 12;
            reasons.push(`Stablecoin inflow regime (${stableRegime.changePct24h?.toFixed(2)}%) — buying pressure`);
        }
        else if (stableRegime.regime === 'outflow') {
            score -= 12;
            reasons.push(`Stablecoin outflow regime (${stableRegime.changePct24h?.toFixed(2)}%) — selling pressure`);
        }
    }
    const confidence = Math.min(Math.abs(score), 100);
    const direction = score >= 50 ? 'strong-buy' :
        score >= 20 ? 'buy' :
    ...
    const isForex = symbol.includes('_') && (symbol.endsWith('_USD') || symbol.endsWith('_JPY') || symbol.includes('AUD'));
