...
export class PairsEngine {
    ...
    openPosition(direction, btcPrice, ethPrice, ratio, spread, zScore, beta, corr) {
    ...
    closePosition(btcPrice, ethPrice, exitSpread, exitZScore, corr, reason) {
        if (!this.position)
            return;
    ...
    getUnrealizedPnl(btcPrice, ethPrice) {
        if (!this.position)
            return 0;
        const pos = this.position;
        let btcPnl;
...
function computeReturnCorrelation(btcSeries, ethSeries) {
    if (btcSeries.length < 3 || ethSeries.length < 3)
        return 0;
    const btcReturns = [];
    const ethReturns = [];
