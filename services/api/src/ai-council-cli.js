...
export function setCouncilRef(fn) {
    _getAiCouncil = fn;
}
export class ClaudeCliProvider {
    ...
    async evaluate(candidate, decisionId) {
        ...
        try {
            const { stdout } = await runProcess(CLAUDE_BIN, ['-p', '--output-format', 'json', '--model', CLAUDE_MODEL], { cwd: WORKSPACE_ROOT, timeoutMs: 30_000, stdin: prompt });
...
export class CodexCliProvider {
    ...
    async evaluate(candidate, decisionId) {
        ...
        try {
            const { stdout } = await runProcess(CODEX_BIN, ['exec', '-m', CODEX_MODEL, '--full-auto', '-'], { cwd: WORKSPACE_ROOT, timeoutMs: 30_000, stdin: prompt });
...
export class GeminiCliProvider {
    ...
    async evaluate(candidate, decisionId) {
        ...
        try {
            const { stdout } = await runProcess(GEMINI_BIN, ['-m', GEMINI_MODEL, '--output-format', 'json', '-p', '-'], { cwd: WORKSPACE_ROOT, timeoutMs: Number(process.env.GEMINI_TIMEOUT_MS ?? 90_000), stdin: prompt });
...
export class PiCliProvider {
    ...
    async evaluate(candidate, decisionId) {
        ...
        try {
            ...
            const { stdout } = await runProcess(PI_BIN, [
                '-p',
                '--mode', 'json',
                '--provider', piProvider,
                '--model', PI_MODEL,
...
async function fetchOllamaWithRetry(url, body, timeoutMs) {
    let lastErr;
...
export class OllamaCliProvider {
    rateLimitedUntil = 0;
...
// ============================================================================
export class Ollama2CliProvider {
    rateLimitedUntil = 0;
    _available = null;
    isRateLimited() {
        if (this._available === false)
...
export function buildRulesDecision(candidate, reason) {
    const action = candidate.score >= 3 ? 'approve' : 'review';
    return {
        provider: 'rules',
        source: 'rules',
        action,
