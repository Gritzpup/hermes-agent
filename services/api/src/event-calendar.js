...
import { fetchUpcomingEarnings } from './finnhub-calendar.js';
import { fetchUpcomingMacroEvents, checkMacroEmbargo } from './trading-economics-calendar.js';
...
async function fetchWithTimeout(input, timeoutMs = 5_000) {
...
export class EventCalendar {
    ...
    economicRefreshInFlight = false;
    finnhubRefreshInFlight = false;
    teRefreshInFlight = false;
    events = [];
    economicEvents = [];
    finnhubEarnings = [];
    teEvents = [];
    ...
    stop() {
        ...
        if (this.economicTimer) {
            clearInterval(this.economicTimer);
            this.economicTimer = null;
        }
        if (this.finnhubTimer) {
            clearInterval(this.finnhubTimer);
            this.finnhubTimer = null;
        }
        if (this.teTimer) {
            clearInterval(this.teTimer);
            this.teTimer = null;
        }
    }
    getSnapshot() {
        ...
        const activeEmbargoes = unique(['BTC-USD', 'ETH-USD', 'SOL-USD', 'XRP-USD', 'PAXG-USD', ...TRACKED_SYMBOLS])
            ...
            .filter((embargo) => embargo.blocked);
    ...
    detectMissingKeys() {
        ...
        if (!readEnv(['TRADING_ECONOMICS_API_KEY']))
            missing.push('TRADING_ECONOMICS_API_KEY');
        return missing;
    }
    getEmbargo(symbol) {
        const now = Date.now();
        ...
        if (finnhubEmbargo.blocked)
            return finnhubEmbargo;
        const allEvents = [...this.events, ...this.economicEvents];
        const activeEvent = allEvents.find((event) => event.symbol === symbol &&
    ...
    async refreshEconomicCalendar() {
        if (this.economicRefreshInFlight)
            return;
        ...
        try {
            ...
            for (const item of payload) {
                if (!item.title || !item.date || item.impact !== 'High')
                    continue;
                ...
                if (eventAtMs + ECONOMIC_POST_EMBARGO_MIN * 60_000 < now)
                    continue;
     ...
     */
    async refreshFinnhubEarnings() {
        if (this.finnhubRefreshInFlight)
            return;
     ...
     */
    checkFinnhubEarningsEmbargo(symbol, nowMs) {
        for (const event of this.finnhubEarnings) {
            if (event.symbol.toUpperCase() !== symbol)
                continue;
...
function normalizeEventDate(date, timeText) {
