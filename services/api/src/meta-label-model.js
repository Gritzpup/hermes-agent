...
function normalizeDirection(value) {
    ...
    if (normalized.includes('strong-buy') || normalized.includes('buy') || normalized.includes('bull'))
        return 'bullish';
    if (normalized.includes('strong-sell') || normalized.includes('sell') || normalized.includes('bear'))
        return 'bearish';
    return 'neutral';
}
...
function inferStyle(entry) {
    const text = `${entry.strategyId ?? ''} ${entry.strategy}`.toLowerCase();
    if (text.includes('revert') || text.includes('mean'))
        return 'mean-reversion';
    if (text.includes('breakout'))
        return 'breakout';
    return 'momentum';
}
function inferAssetClass(entry) {
    if (entry.assetClass)
    ...
    if (symbol.includes('_')) {
        if (symbol.startsWith('USB'))
            return 'bond';
        if (symbol.startsWith('BCO') || symbol.startsWith('WTICO'))
            return 'commodity';
        return 'forex';
    }
    return 'equity';
...
function candidateTokens(candidate) {
    ...
    if (Array.isArray(candidate.entryTags)) {
        for (const tag of candidate.entryTags) {
            if (tag.trim())
                tokens.add(`entry-tag:${tag.trim().toLowerCase()}`);
        }
    }
    ...
    for (const tag of candidate.tags) {
        if (tag.trim())
            tokens.add(`tag:${tag.trim().toLowerCase()}`);
    }
    return Array.from(tokens.values()).sort();
}
function journalToCandidate(entry) {
    return {
...
export function predictWithModel(model, candidate) {
...
export function predictMetaLabel(entries, candidate) {
...
function evaluateWalkForward(entries) {
...
function rankTokenEdges(model, direction) {
    const edges = [];
...
export function buildMetaLabelModelSnapshot(entries, candidates) {
    const filtered = entries.filter((entry) => (entry.lane ?? 'scalping') === 'scalping' && entry.realizedPnl !== 0);
    const model = buildModel(filtered);
    return {
