...
import { ClaudeCliProvider, CodexCliProvider, GeminiCliProvider, PiCliProvider, OllamaCliProvider, Ollama2CliProvider, buildRulesDecision, setCouncilRef, } from './ai-council-cli.js';
...
export class AiCouncil {
    ...
    inFlight = false;
    timer = null;
    lastRealDecision = null;
    // Fix E — degraded tracking: count rules-provider votes across recent decisions
    _recentPanelSnapshots = [];
    static _DEGRADED_WINDOW = 10;
    static _DEGRADED_THRESHOLD = 0.4;
    constructor() {
    ...
    recordTrace(trace) {
        ...
        this.logQueue = this.logQueue.then(async () => {
            try {
                if (!fs.existsSync(path.dirname(TRACE_LOG_PATH)))
    ...
    async evaluateWithRotation(preferred, candidate, decisionId) {
    ...
    async drain() {
        ...
        try {
            ...
            const challenger = challengerRaw.status === 'fulfilled'
                ? challengerRaw.value : buildRulesDecision(cached.candidate, 'Codex failed');
            const tertiary = tertiaryRaw.status === 'fulfilled'
            ...
            if (this.metaLabelProvider.isReady() &&
     ...
     */
    combineLocalOnly(piResult, ollamaResult, ollama2Result, candidate) {
    ...
    combine(primary, challenger, tertiary, piResult, candidate) {
        ...
        const strongestReject = rejectVotes.reduce((best, vote) => {
            if (!best)
                return vote;
     ...
     */
    combineSix(primary, challenger, tertiary, piResult, ollamaResult, ollama2Result, candidate) {
        ...
        const strongestReject = rejectVotes.reduce((best, v) => !best || v.confidence > best.confidence ? v : best, null);
     ...
     *   - Claude primary with high conviction overrides mixed panels
     */
    combineSeven(primary, challenger, tertiary, piResult, ollamaResult, ollama2Result, metaLabelResult, candidate) {
    ...
    buildRulesFallback(candidate, reason) {
        const primary = {
            provider: 'rules',
            source: 'rules',
            action: candidate.score >= 3 ? 'approve' : 'review',
