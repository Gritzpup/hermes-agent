...
export class HistoricalContextStore {
    ...
    recordRegimeChange(regime, previousRegime, trigger, fng, btcPrice) {
        ...
        if (this.regimeHistory.length > 500)
            this.regimeHistory.splice(0, this.regimeHistory.length - 500);
        this.persist();
    }
    ...
    getRegimeTransitionHistory(from, to) {
    ...
    buildSummary() {
        ...
        if (cpi)
            parts.push(`CPI: ${cpi.value} (${cpi.date})`);
        const fedRate = this.getLatestMacro('FEDFUNDS');
        if (fedRate)
            parts.push(`Fed Funds: ${fedRate.value}% (${fedRate.date})`);
        const unemployment = this.getLatestMacro('UNRATE');
        if (unemployment)
            parts.push(`Unemployment: ${unemployment.value}% (${unemployment.date})`);
        const spread = this.getLatestMacro('T10Y2Y');
        if (spread)
            parts.push(`10Y-2Y Spread: ${spread.value}% (${spread.date})${spread.value < 0 ? ' [INVERTED - recession signal]' : ''}`);
        const fngTrend = this.getFngTrend();
        if (fngTrend)
            parts.push(`F&G: ${fngTrend.current} (7d avg ${fngTrend.avg7d}, ${fngTrend.direction})`);
    ...
    load() {
        try {
            if (fs.existsSync(MACRO_PATH)) {
                this.macroIndicators = JSON.parse(fs.readFileSync(MACRO_PATH, 'utf8'));
            }
        }
        catch { /* corrupt file, start fresh */ }
        try {
            if (fs.existsSync(REGIME_PATH)) {
                this.regimeHistory = JSON.parse(fs.readFileSync(REGIME_PATH, 'utf8'));
            }
        }
        catch { /* corrupt file, start fresh */ }
        try {
            if (fs.existsSync(FNG_HISTORY_PATH)) {
                this.fngHistory = JSON.parse(fs.readFileSync(FNG_HISTORY_PATH, 'utf8'));
            }
        }
        catch { /* corrupt file, start fresh */ }
    }
