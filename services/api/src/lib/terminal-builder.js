import {} from '@hermes/contracts';
import {} from './types-routes.js';
import { MARKET_DATA_URL, RISK_ENGINE_URL, BROKER_ROUTER_URL, REVIEW_LOOP_URL, BACKTEST_URL, STRATEGY_LAB_URL } from './constants.js';
import { fetchJson, fetchArrayJson, getServiceHealthSnapshot } from './utils-http.js';
...
export function compactTerminalLines(lines) {
    return lines.filter((line) => typeof line === 'string' && line.trim().length > 0).slice(0, 20);
}
export function buildTerminalPane(id, label, status, summary, lines) {
    return {
        id,
        label,
        status,
...
export function buildTerminalFallbackSnapshot(error) {
    return {
...
export async function buildTerminalSnapshot(deps, overrides) {
    const [health, marketHealth, marketMicrostructure, riskSettings, brokerState, brokerReports, reviews, reviewClusters, copySleeve, macroPreservation, secEdgarIntel, strategyBest, strategyHistory, strategyHealth] = await Promise.all([
        ...
        fetchJson(MARKET_DATA_URL, '/health'),
        overrides?.marketMicrostructure ?? fetchJson(MARKET_DATA_URL, '/microstructure'),
        fetchJson(RISK_ENGINE_URL, '/settings'),
        fetchJson(BROKER_ROUTER_URL, '/account'),
        overrides?.brokerReports ?? fetchJson(BROKER_ROUTER_URL, '/reports'),
        overrides?.reviews ?? fetchArrayJson(REVIEW_LOOP_URL, '/reviews'),
        fetchJson(REVIEW_LOOP_URL, '/clusters'),
        overrides?.copySleeve ?? fetchJson(BACKTEST_URL, '/copy-sleeve', 5_000),
        overrides?.macroPreservation ?? fetchJson(BACKTEST_URL, '/macro-preservation', 5_000),
        overrides?.secEdgarIntel ?? getSecEdgarIntel().getSnapshot(),
        fetchJson(STRATEGY_LAB_URL, '/best'),
        fetchArrayJson(STRATEGY_LAB_URL, '/history'),
        overrides?.strategyHealth ?? fetchJson(STRATEGY_LAB_URL, '/health')
    ]);
            ...
            : latestDecision.status === 'evaluating'
                ...
                : latestDecision.status === 'error'
                    ...
                    : latestDecision.panel?.length
                        ...
                        : [latestDecision.primary, latestDecision.challenger].filter((vote) => Boolean(vote)).map((vote) => `${voteLabel(vote)}:${vote.action} ${vote.confidence}%`).join(' · ');
    const formatVoteLine = (label, vote) => {
        if (!vote) {
    ...
    const latestMinimax = hasFinalCouncilVotes ? (latestDecision.panel?.find((v) => v?.provider === 'pi') ?? null) : null;
    const latestOllamaFinance = hasFinalCouncilVotes ? (latestDecision.panel?.find((v) => typeof v?.provider === 'string' && v.provider.includes('hermes3')) ?? null) : null;
    const latestOllamaQwen = hasFinalCouncilVotes ? (latestDecision.panel?.find((v) => typeof v?.provider === 'string' && v.provider.includes('qwen35')) ?? null) : null;
    ...
    const latestTrace = latestDecision?.status === 'complete' ? latestDecisionTrace : null;
    const terminalTimestamp = new Date().toISOString();
    const asString = (val) => (typeof val === 'string' ? val : null);
    ...
    return {
        ...
        terminals: [
            buildTerminalPane('api', 'Hermes API', healthMap.get('api')?.status ?? 'healthy', `${paperDesk.activeAgents} active agents · ${paperDesk.totalTrades} trades · win ${paperDesk.winRate.toFixed(1)}%`, [
                latestDecision
            ...
            buildTerminalPane('ai-council', 'AI Council', councilStatus.inFlight || councilStatus.queued > 0 ? 'warning' : latestDecision?.finalAction === 'reject' ? 'warning' : 'healthy', latestTrace
            ...
            buildTerminalPane('claude-terminal', 'Claude', latestPrimary ? (latestPrimary.error ? 'critical' : 'healthy') : 'warning', latestPrimary
            ...
            buildTerminalPane('codex-terminal', 'Codex', latestChallenger ? (latestChallenger.error ? 'critical' : 'healthy') : 'warning', latestChallenger
            ...
            buildTerminalPane('gemini-terminal', 'Gemini', latestGemini ? (latestGemini.error ? 'critical' : 'healthy') : 'warning', latestGemini
            ...
            buildTerminalPane('minimax-terminal', 'MiniMax', latestMinimax ? (latestMinimax.error ? 'critical' : 'healthy') : 'warning', latestMinimax
            ...
            buildTerminalPane('ollama-terminal', 'Ollama', (latestOllamaFinance || latestOllamaQwen)
                ? ((latestOllamaFinance?.error || latestOllamaQwen?.error) ? 'critical' : 'healthy')
                : 'warning', (() => {
                const parts = [];
                if (latestOllamaFinance)
                    parts.push(`[finance-llama-8b] ${latestOllamaFinance.action} ${latestOllamaFinance.confidence}%`);
                if (latestOllamaQwen)
                    parts.push(`[qwen3.5-9b] ${latestOllamaQwen.action} ${latestOllamaQwen.confidence}%`);
                ...
                latestDecision ? `[candidate] ${latestDecision.symbol} · ${latestDecision.agentName}` : '[candidate] No active candidate.',
                ...(() => {
                    const nonCouncil = getRecentOllamaActivity(20).filter((e) => !e.source.startsWith('ai-council'));
                    if (nonCouncil.length === 0)
                        return [];
                    return nonCouncil.slice(-6).map((e) => `[${e.source}] ${e.model} ${e.status} ${e.latencyMs != null ? `${e.latencyMs}ms` : '—'} · ${e.status === 'error' ? (e.errorPreview ?? 'error') : e.responsePreview}`);
                })()
            ]),
            // Meta-label model pane (7th council voter)
            (() => {
                const latestMetaLabel = hasFinalCouncilVotes
                    ? (latestDecision.panel?.find((v) => v?.provider === 'meta-label') ?? null)
                    : null;
                const metaLabelMeta = deps.aiCouncil.getStatus();
                return buildTerminalPane('meta-label', 'Meta-Label', latestMetaLabel ? 'healthy' : metaLabelMeta.enabled ? 'warning' : 'warning', latestMetaLabel
            ...
            buildTerminalPane('market-data', 'Market Data', healthMap.get('market-data')?.status ?? 'warning', `${microSnapshots.length} live microstructure feeds · ${asString(marketHealth?.message) ?? 'market polling current'}`, [
            ...
            buildTerminalPane('risk-engine', 'Risk Engine', healthMap.get('risk-engine')?.status ?? 'warning', `Trade cap ${riskSettings ? `$${riskSettings.riskCaps.maxTradeNotional.toFixed(0)}` : 'n/a'} · drawdown gate ${riskSettings ? `${riskSettings.riskCaps.maxDrawdownPct.toFixed(1)}%` : 'n/a'}`, [
                riskSettings
            ...
            buildTerminalPane('broker-router', 'Broker Router', healthMap.get('broker-router')?.status ?? 'warning', `${brokerAccounts.length} account snapshots · ${brokerExecutions.length} execution reports`, [
            ...
            (() => {
                ...
                return buildTerminalPane('live-safety', 'Live Capital', statusMap[snap.status] ?? 'warning', `status=${snap.status} · today ${todayCount}/${snap.maxTradesPerDay} trades · P&L $${snap.liveTotalPnl.toFixed(2)}`, [
            ...
            buildTerminalPane('review-loop', 'Review Loop', healthMap.get('review-loop')?.status ?? 'warning', `${reviews.length} reviews · ${deps.paperEngine.getJournal().length} journal entries · ${learningLogCount} learning logs · ${laneLearningLogCount} lane logs`, [
                latestReview
            ...
            buildTerminalPane('backtest', 'Backtest Service', healthMap.get('backtest')?.status ?? 'warning', `${copySleeve ? copySleeve.managerName : 'Copy sleeve'} · ${macroPreservation ? macroPreservation.regime : 'macro snapshot unavailable'}`, [
                copySleeve
            ...
            (() => {
                const edgar = secEdgarIntel;
                const signals = edgar?.signals ?? [];
                const top3 = signals.slice(0, 3);
                const newBuys = signals.filter((s) => s.action === 'new');
                const status = edgar?.errors?.length
                    ? (edgar.errors.length >= edgar.ciksQueried ? 'critical' : 'warning')
                    : 'healthy';
                return buildTerminalPane('copy-sleeve', 'Copy Sleeve', status, `${signals.length} signals · ${newBuys.length} new buys · quarter ${edgar?.quarterKey ?? 'n/a'}`, [
                    edgar?.lastPollAt
                    ...
                    ...top3.map((s) => `[${s.action}] ${s.filer} → ${s.symbol} ${s.percentOfPortfolio.toFixed(1)}%`),
                    signals.length > 3 ? `[more] +${signals.length - 3} more signals` : null,
                ].filter(Boolean));
            })(),
            buildTerminalPane('strategy-lab', 'Strategy Lab', healthMap.get('strategy-lab')?.status ?? 'warning', `${asString(strategyHealth?.status) ?? 'ready'} · ${strategyHistory.length} history entries`, [
                strategyBest
            ...
            (() => {
                ...
                return buildTerminalPane('strategy-director', 'Strategy Director', status, latest
                    ...
                    latest
                        ...
                        : '[reasoning] pending.',
                    ...(latest?.playbookApplications?.slice(0, 8).map((p) => `[playbook] ${p.agentId} → '${p.templateName}' (${p.regime})`) ?? []),
                    ...(latest?.agentAdjustments?.slice(0, 8).map((a) => `[fine-tune] ${a.agentId}.${a.field}: ${a.oldValue} → ${a.newValue} — ${a.reason?.slice(0, 80) ?? ''}`) ?? []),
                    ...(latest?.symbolChanges?.slice(0, 6).map((s) => `[symbol] ${s.action} ${s.symbol} on ${s.broker} — ${s.reason?.slice(0, 80) ?? ''}`) ?? []),
                    latest
