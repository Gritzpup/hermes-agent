...
export function getPositionDirection(engine: any, position: any): 'long' | 'short' {
  return position?.direction ?? 'long';
}

export function getPositionUnrealizedPnl(engine: any, position: any, markPrice: number): number {
...
export function maybeTrailBrokerStop(engine: any, agent: any, symbol: any): void {
...
export function getFeeRate(engine: any, assetClass: string): number {
  if (assetClass === 'crypto') return 0.0006; // 6bps
  if (assetClass === 'equity') return 0.0001; // 1bp
  return 0.0003; // 3bps
}

export function roundTripFeeBps(engine: any, assetClass: string): number {
  return engine.getFeeRate(assetClass) * 2 * 10_000;
}

export function computeDynamicStop(engine: any, fillPrice: number, agent: any, symbol: any, direction: string): number {
  const mult = direction === 'short' ? 1 : -1;
  const stopBps = agent.config.stopBps;
  return fillPrice * (1 + (mult * stopBps) / 10_000);
}

export function computeDynamicTarget(engine: any, fillPrice: number, agent: any, symbol: any, direction: string): number {
  const mult = direction === 'short' ? -1 : 1;
  const targetBps = agent.config.targetBps;
  return fillPrice * (1 + (mult * targetBps) / 10_000);
}

export function resolveEntryDirection(engine: any, agent: any, symbol: any, score: number, intel?: any): string {
  if (intel?.direction === 'buy' || intel?.direction === 'strong-buy') return 'long';
  if (intel?.direction === 'sell' || intel?.direction === 'strong-sell') return 'short';
  return score > 0 ? 'long' : 'short';
}

export function computeGrossPnl(engine: any, position: any, exitPrice: number, quantity: number): number {
  const direction = engine.getPositionDirection(position);
  if (direction === 'short') {
...
export function getSessionBucket(engine: any, isoTs?: string): string {
  const date = isoTs ? new Date(isoTs) : new Date();
  const hours = date.getUTCHours();
  if (hours >= 0 && hours < 8) return 'asia';
  if (hours >= 8 && hours < 16) return 'europe';
  return 'america';
}

export function getVolatilityBucket(engine: any, symbol: any): 'low' | 'medium' | 'high' {
  // Logic to classify volatility based on recent returns
  return 'medium';
}

export function noteTradeOutcome(engine: any, agent: any, symbol: any, realized: number, reason: string): void {
...
export function getDeskEquity(engine: any): number {
  return Array.from(engine.agents.values()).reduce((sum: number, agent: any) => sum + engine.getAgentEquity(agent), 0);
}

export function getBenchmarkEquity(engine: any): number {
  const pilotSymbols = new Set(engine.getDeskAgentStates().map((a: any) => a.config.symbol));
  const scopedSymbols = Array.from(engine.market.values()).filter((s: any) => pilotSymbols.has(s.symbol));
  const symbols = scopedSymbols.filter((s: any) => engine.hasTradableTape(s));
  ...
  const validReturns = benchmarkSymbols
    .filter((s: any) => s.price > 0 && s.openPrice > 0)
    .map((s: any) => (s.price - s.openPrice) / s.openPrice);
...
export function computeHalfKelly(engine: any, agent: any): number {
  const outcomes = (agent.recentOutcomes ?? []).slice(-30);
  if (outcomes.length < 10) return agent.config.sizeFraction;
  const wins = outcomes.filter(o => o > 0);
  const losses = outcomes.filter(o => o < 0);
  if (losses.length === 0) return Math.min(agent.config.sizeFraction * 1.5, 0.15);
...
export function breachesCrowdingLimit(engine: any, symbol: string, proposedNotional: number): boolean {
...
export function getEffectiveLeverage(engine: any): number {
