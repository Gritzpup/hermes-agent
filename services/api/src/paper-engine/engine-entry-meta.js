import { predictWithModel } from '../meta-label-model.js';
import { average, clamp, pickLast, round } from '../paper-engine-utils.js';
import { estimateExpectedGrossEdgeBps, estimateRoundTripCostBps } from '../fee-model.js';
export function getMetaLabelDecision(engine, agent, symbol, score, intel) {
    ...
    const heuristicProbability = clamp(0.18
    ...
    const completedTrades = agent.trades ?? 0;
    ...
    let probability = (heuristicProbability * heuristicWeight
        + contextual.posterior * contextualWeight
        + trained.posterior * trainedWeight) / Math.max(weightSum, Number.EPSILON);
...
export function getContextualMetaSignal(engine, agent, symbol, intel) {
    ...
    const contextMatches = entries.filter((entry) => entry.symbol === symbol.symbol
    ...
    const regimeMatches = entries.filter((entry) => (entry.regime ?? 'unknown') === regime
        && engine.normalizeFlowBucket(entry.orderFlowBias ?? 'neutral') === flowBucket);
    const summarize = (group, priorWins, priorLosses) => {
    ...
    const weightedPosterior = clamp(global.posterior * 0.15
        + regimeStats.posterior * 0.2
        + symbolStats.posterior * 0.25
        + contextStats.posterior * 0.25
        + exactStats.posterior * 0.15, 0.05, 0.98);
...
export function buildEntryMeta(engine, agent, symbol, score) {
...
export function buildMetaCandidate(engine, agent, symbol, intel) {
    const context = buildJournalContext(engine, symbol);
...
export function buildJournalContext(engine, symbol) {
    const intel = engine.marketIntel.getCompositeSignal(symbol.symbol);
