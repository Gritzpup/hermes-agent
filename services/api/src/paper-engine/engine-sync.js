...
export function isHermesBrokerOrderId(engine, orderId) {
    return typeof orderId === 'string' && orderId.startsWith('paper-');
}
export function matchesHermesBrokerOrderForAgent(engine, agent, orderId) {
    return typeof orderId === 'string' && orderId.startsWith(`paper-${agent.config.id}-`);
}
export function isOwnedBrokerFill(engine, fill) {
...
export function isOwnedBrokerJournal(engine, entry) {
    const agent = engine.getBrokerPaperAgentByStrategy(entry.strategy);
    if (!agent)
        return true;
    return entry.source === 'broker';
}
export function isRestoredExternalBrokerJournal(engine, entry) {
...
export function isRestoredExternalBrokerExitFill(engine, fill, journalEntries) {
    const agent = engine.agents.get(fill.agentId);
    ...
    return journalEntries.some(entry => {
        if (!engine.isRestoredExternalBrokerJournal(entry))
            return false;
...
export function hasMatchingOwnedBrokerEntryFill(engine, fill, fills) {
    if (fill.side !== 'sell')
        return true;
    const fillTimestamp = Date.parse(fill.timestamp);
    return fills.some(candidate => {
        if (candidate.agentId !== fill.agentId || candidate.symbol !== fill.symbol || candidate.side !== 'buy')
            return false;
        if (!engine.isOwnedBrokerFill(candidate))
            return false;
        const candidateTimestamp = Date.parse(candidate.timestamp);
        return candidateTimestamp <= fillTimestamp;
    });
}
export function getBrokerSellQuantity(engine, agent, trackedQuantity) {
...
export function toBrokerPaperAccountState(engine, snapshot) {
...
export function sanitizeBrokerPaperRuntimeState(engine) {
...
export function hasHermesBrokerPosition(engine, agent, brokerOrders) {
...
export function syncMarketFromRuntime(engine, recordHistory) {
    const runtime = engine.loadMarketDataState();
    if (!runtime) {
        if (engine.tick <= 3)
            console.log(`[paper-engine] tick ${engine.tick}: market-data runtime not available yet`);
        return false;
    }
    ...
    if (engine.tick <= 3)
        console.log(`[paper-engine] tick ${engine.tick}: loaded ${snapshotMap.size} market snapshots`);
...
export function applyMarketSnapshot(engine, symbol, snapshot, recordHistory) {
    // Don't overwrite good data with a zero-price snapshot from another broker
    if (snapshot.lastPrice <= 0 && symbol.price > 0 && symbol.tradable)
        return;
    ...
    const tradable = snapshot.tradable ?? (snapshot.status === 'live'
    ...
    const switchedToRuntimeTape = (previousSourceMode === 'simulated' || previousSourceMode === 'mock')
        && snapshot.source !== 'simulated'
        && snapshot.source !== 'mock';
...
export function getLatestBrokerPositions(engine) {
