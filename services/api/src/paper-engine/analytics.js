 ...
 */
export function buildMistakeProfile(recentOutcomes, recentHoldTicks, recentEntries) {
 ...
 * PURE — zero this. references.
 */
export function applyMistakeDrivenRefinement(profile, config, improvementBias) {
    if (profile.dominant === 'clean') {
        return { bias: 'hold-steady', note: 'No dominant mistake pattern.' };
    }
    let bias = improvementBias;
    let note = '';
 ...
 * PURE — zero this. references.
 */
export function toBrokerPaperAccountState(account, broker) {
    if (broker === 'coinbase-live') {
        ...
        const cash = round(accounts.reduce((sum, item) => {
            ...
            if (currency !== 'USD' && currency !== 'USDC')
                return sum;
            const val = record.available_balance;
