...
export function describeWatchState(engine, style, symbol, score) {
...
export function describeAiState(engine, decision) {
...
export function normalizeFlowBucket(engine, direction) {
...
export function getConfidenceBucket(engine, confidence) {
    if (confidence >= 70)
        return 'high';
    if (confidence >= 35)
        return 'medium';
...
export function getSpreadBucket(engine, spreadBps, limitBps) {
...
export function entryNote(engine, style, symbol, score) {
    if (style === 'momentum') {
...
export function getEntryScore(engine, style, shortReturn, mediumReturn, symbol) {
    if (symbol.price <= 0 || symbol.history.length < 2)
        return 0;
    ...
    if (rsi2 !== null) {
        if (style === 'mean-reversion') {
            ...
            if (rsi2 < 10)
                ...
                rsi2Bonus = -1.0; // wrong side for mean-reversion
        }
        else {
            // For momentum/breakout, overbought momentum confirmation
            if (rsi2 > 65 && rsi2 < 85)
                rsi2Bonus = 0.8; // strong momentum, not yet exhausted
            else if (rsi2 < 15)
                rsi2Bonus = -1.5; // too oversold, don't chase long
        }
    }
    ...
    let stochBonus = 0;
    if (stoch) {
        if (style === 'mean-reversion' && stoch.crossover === 'bullish' && stoch.k < 30)
            stochBonus = 1.5;
        else if (style === 'momentum' && stoch.crossover === 'bullish' && stoch.k > 50)
            stochBonus = 1.0;
...
export function entryThreshold(engine, style, assetClass) {
    let base;
    if (style === 'breakout') {
        base = 1.85;
    }
...
export function exitThreshold(engine, style) {
    return -999;
}
...
export function fastPathThreshold(engine, style, assetClass) {
    return entryThreshold(engine, style, assetClass) + (style === 'breakout' ? 0.9 : 0.6);
}
export function brokerRulesFastPathThreshold(engine, agent, symbol) {
