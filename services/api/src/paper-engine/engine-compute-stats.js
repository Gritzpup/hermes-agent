...
let _cachedRegime = 'unknown';
const _regimeSubscriber = redis.duplicate();
_regimeSubscriber.subscribe(TOPICS.REGIME_UPDATE, (err) => {
    if (err)
        console.error('[engine-compute-stats] Failed to subscribe to REGIME_UPDATE:', err?.message ?? err);
});
_regimeSubscriber.on('message', (_channel, message) => {
    try {
...
export function buildRegimeKpis(engine) {
...
export function refreshCapitalAllocation(engine, snapshot) {
    ...
    const capitalManager = new CapitalManager({
        ...
        hasTradableTape: (symbol) => engine.hasTradableTape(symbol),
        getMetaJournalEntries: (limit) => engine.getMetaJournalEntries(limit),
        getRecentJournalEntries: (agent, symbol, limit) => engine.getRecentJournalEntries(agent, symbol, limit),
        buildMistakeProfile: (agent, symbol, entries) => engine.buildMistakeProfile(agent, symbol, entries),
        getRegime: _getRegime,
    });
    capitalManager.refreshAllocation(snapshot);
}
export function buildDeskAnalytics(engine) {
    ...
    const totalOpenRisk = Array.from(engine.agents.values()).reduce((sum, agent) => {
        if (!agent.position)
            return sum;
...
export function buildExecutionBands(engine) {
