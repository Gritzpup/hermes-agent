...
export function getExecutionQualityByBroker(engine) {
    const journal = engine.getMetaJournalEntries().slice(-200);
    const brokers = ['alpaca-paper', 'oanda-rest', 'coinbase-live'];
    return brokers.map((broker) => {
        ...
        const avgLatencyMs = sampleCount > 0
            ? average(rows.map((entry) => Number.isFinite(entry.latencyMs) ? entry.latencyMs : 0))
            : 0;
...
export function formatBrokerLabel(engine, broker) {
    switch (broker) {
        case 'coinbase-live':
...
export function buildMarketTape(engine, journalRows) {
    const visibleFills = engine.getVisibleFills();
    // Use agent's configured broker for routing, not market data source
    const agentBrokerMap = new Map(Array.from(engine.agents.values()).map((a) => [a.config.symbol, a.config.broker]));
    return Array.from(engine.market.values()).map((symbol) => {
        ...
        if (journalRows) {
            ...
            if (symbolJournals.length > 0) {
                lastTradeAt = symbolJournals.reduce((latest, r) => (!latest || r.exitAt > latest) ? r.exitAt : latest, '');
            }
        }
        // Fallback to visibleFills for backward compat
        if (lastTradeAt === null) {
            const fillLookup = visibleFills.filter((f) => f.symbol === symbol.symbol);
            if (fillLookup.length > 0) {
                lastTradeAt = fillLookup.sort((a, b) => b.timestamp.localeCompare(a.timestamp))[0]?.timestamp ?? null;
            }
        }
        return {
...
export function analyzeSignals(engine) {
    ...
    const symbols = Array.from(engine.market.values()).filter((s) => s.price > 0 && s.openPrice > 0);
...
export function getDataSources(engine) {
    const tradableSymbols = Array.from(engine.market.values())
        .filter((symbol) => engine.hasTradableTape(symbol))
        .map((symbol) => symbol.symbol);
    const blockedSymbols = Array.from(engine.market.values())
        .filter((symbol) => !engine.hasTradableTape(symbol))
        .map((symbol) => `${symbol.symbol} (${engine.describeTapeFlags(symbol)})`);
...
export function getLiveReadinessReport(engine) {
    const agents = Array.from(engine.agents.values()).map((agent) => toLiveReadiness(engine, agent));
    const blockers = agents.filter((a) => !a.eligible).map((a) => `${a.agentId}: ${a.reason ?? 'not eligible'}`);
    return {
...
export function toLiveReadiness(engine, agent) {
...
export function toAgentSnapshot(engine, agent, journalRows) {
    ...
    const rows = journalRows
        ...
        : readJsonLines(JOURNAL_LEDGER_PATH).filter((e) => e.strategyId === agent.config.id);
    ...
    const directionBias = agent.position
        ...
        : 'neutral';
    const executionQualityScore = getExecutionQualityByBroker(engine).find((row) => row.broker === agent.config.broker)?.score ?? 0;
    ...
    const operationalGate = engine.circuitBreakerLatched
        ...
        : engine.operationalKillSwitchUntilMs > Date.now()
            ...
            : 'clear';
    const lane = agent.config.id.startsWith('agent-mk-') || agent.config.id.startsWith('maker-') ? 'maker' :
...
export function getDeskAgentStates(engine) {
    ...
    const paperTradeAgents = Array.from(engine.agents.values()).filter((agent) => activeStates.includes(agent.status) || agent.position !== null);
    const brokerPaperPilots = Array.from(engine.agents.values()).filter((agent) => agent.config.executionMode === 'broker-paper' && agent.config.autonomyEnabled);
...
export function getVisibleFills(engine) {
    const deskAgentIds = new Set(engine.getDeskAgentStates().map((agent) => agent.config.id));
    return engine.fills.filter((fill) => deskAgentIds.has(fill.agentId));
}
export function getDeskStartingEquity(engine) {
    ...
    const agentStartingTotal = engine.getDeskAgentStates().reduce((sum, agent) => sum + agent.startingEquity, 0);
...
export function getDeskEquity(engine) {
    ...
    if (_journalCache && _journalCache.rows.length > 0) {
        const cbRows = _journalCache.rows.filter((r) => r.broker === 'coinbase-live' && r.exitAt);
        coinbasePnl = round(cbRows.reduce((s, r) => s + (r.realizedPnl ?? 0), 0), 2);
    }
    else {
        // Fallback: use scalper agent PnL (misses Grid/Maker but better than nothing)
        const cbAgents = Array.from(engine.agents.values()).filter((a) => a.config.broker === 'coinbase-live');
        coinbasePnl = round(cbAgents.reduce((s, a) => s + (a.realizedPnl ?? 0), 0), 2);
    }
    ...
    const agentEquityTotal = engine.getDeskAgentStates().reduce((sum, agent) => sum + engine.getAgentEquity(agent), 0);
    if (brokerTotal < agentEquityTotal && agentEquityTotal > 0) {
        return round(agentEquityTotal, 2);
    }
    return round(brokerTotal, 2);
}
export function getMarketSnapshots(engine) {
    return Array.from(engine.market.values()).map((symbol) => ({
        symbol: symbol.symbol,
...
export function computeLaneRollups(entries) {
    ...
    const LANES = ['maker', 'grid', 'pairs', 'scalping'];
    return LANES.map((lane) => {
        ...
        const lastTradeAt = rows.length > 0
            ? rows.reduce((latest, r) => (!latest || r.exitAt > latest) ? r.exitAt : latest, null)
            : null;
        return { lane, trades, wins, losses, winRate: round(winRate, 1), realizedPnl: round(realizedPnl, 2), lastTradeAt };
    });
}
export function computeBrokerRollups(entries) {
    const cutoff = new Date();
    cutoff.setDate(cutoff.getDate() - 30);
    const cutoffStr = cutoff.toISOString();
    const recent = entries.filter((e) => e.exitAt >= cutoffStr);
...
function buildJournalFills(journalRows, limit = 50) {
...
function computeFirmRegime(engine) {
    const counts = {};
    let total = 0;
...
export function getSnapshot(engine) {
    ...
    if (!_journalCache || now - _journalCache.ts > 1000) {
        _journalCache = { rows: readJsonLines(JOURNAL_LEDGER_PATH), ts: now };
    }
    // Phase H2: Filter quarantined entries for analytics to avoid KPI pollution.
    const journalRows = _journalCache.rows.filter((entry) => !entry.exitReason || !QUARANTINED_EXIT_REASONS.has(entry.exitReason));
    const agents = agentStates.map((agent) => engine.toAgentSnapshot(agent, journalRows));
    ...
    const realizedPnl = round(journalRows.reduce((sum, entry) => sum + entry.realizedPnl, 0), 2);
    ...
    const todayEntries = journalRows.filter((entry) => {
        if (!entry.exitAt)
            return false;
    ...
    const totalDayPnl = round(todayEntries.reduce((sum, entry) => sum + entry.realizedPnl, 0), 2);
    ...
    const totalTrades = agents.reduce((sum, agent) => sum + (agent.totalTrades ?? agent.trades ?? 0), 0);
    const totalWins = agents.reduce((sum, agent) => {
    ...
    return {
        ...
        totalTrades,
        totalWins,
        totalLosses: agents.reduce((sum, agent) => sum + (agent.losses ?? 0), 0),
        ...
        aiCouncil: (() => {
            ...
            for (const t of traces) {
                ...
                if (!d.reason && t.parsedThesis)
                    d.reason = t.parsedThesis.slice(0, 120);
            }
...
export function getPositions(engine) {
    return Array.from(engine.agents.values())
        ...
        const unrealizedPnl = round(position ? engine.getPositionUnrealizedPnl(position, markPrice) : 0, 2);
...
export function getRiskControlSnapshot(engine) {
    return {
        circuitBreaker: {
...
export function getOpportunitySnapshot(engine) {
    return {
        asOf: new Date().toISOString(),
        ...
        selectedByAssetClass: Object.fromEntries(engine.selectedScalpByAssetClass.entries()),
...
export function hasTradableTape(engine, symbol) {
    if (!symbol)
        return false;
    return ((symbol.marketStatus === 'live' || symbol.marketStatus === 'delayed')
...
export function describeTapeFlags(engine, symbol) {
...
export function toCandles(engine, history) {
...
export function normalizePresentationState(engine) {
