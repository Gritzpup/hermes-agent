...
type WriteTask = { path: string; line: string; pairId?: string };
...
const pendingPairs = new Map<string, { pathA: string; lineA: string; pathB: string; lineB: string }>();
 ...
 */
export function replayOrphanedPairs(): void {
 ...
 */
export function enqueueAppendPaired(
  pathA: string,
  lineA: string,
  pathB: string,
  lineB: string
...
async function flush(): Promise<void> {
  ...
  try {
    while (queue.length > 0) {
      ...
      const pairedTasks = new Map<string, WriteTask[]>();
      const soloTasks: WriteTask[] = [];
