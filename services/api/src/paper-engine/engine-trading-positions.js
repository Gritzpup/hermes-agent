...
const recentStopouts = [];
let correlatedBreakerUntil = null;
function recordStopout(symbol) {
    const now = Date.now();
...
export function isCorrelatedBreakerActive() {
...
export async function manageOpenPosition(engine, agent, symbol, score) {
    ...
    const severeAdverse = newsSignal?.confidence >= 75 &&
        ((direction === 'long' && newsSignal?.direction === 'bearish') ||
            (direction === 'short' && newsSignal?.direction === 'bullish'));
    if (severeAdverse && holdTicks >= 2) {
        await closePosition(engine, agent, symbol, `adverse-news auto-flatten (${newsSignal.direction}, conf ${newsSignal.confidence}%)`);
        return;
    }
...
export async function openPosition(engine, agent, symbol, score) {
    ...
    agent._signalAt = signalAt;
    agent._signalSymbol = symbol.symbol;
...
export async function closePosition(engine, agent, symbol, reason, forcePnl) {
...
export function updateArbAgent(engine, agent, symbol) {
    ...
    const counterAgent = Array.from(engine.agents.values()).find((other) => other.config.symbol === agent.config.symbol && other.config.broker === counterBroker);
