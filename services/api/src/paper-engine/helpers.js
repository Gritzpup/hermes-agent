...
export function getSessionBucket(isoTs = new Date().toISOString()) {
...
export function getVolatilityBucket(volatility) {
    if (volatility >= 0.02)
        return 'high';
    if (volatility >= 0.008)
        return 'medium';
...
export function getSymbolCluster(assetClass) {
    if (assetClass === 'commodity' || assetClass === 'commodity-proxy')
        return 'commodity';
...
export function getClusterLimitPct(cluster) {
    if (cluster === 'crypto')
        return 45;
    if (cluster === 'equity')
        return 35;
...
export function pushPoint(target, value, limit = HISTORY_LIMIT) {
