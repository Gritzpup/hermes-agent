import { CRYPTO_MAX_ENTRY_SPREAD_BPS, CRYPTO_MAX_EST_SLIPPAGE_BPS, CRYPTO_MIN_BOOK_DEPTH_NOTIONAL, DAILY_CIRCUIT_BREAKER_DD_PCT, WEEKLY_CIRCUIT_BREAKER_DD_PCT, PER_PAIR_DAILY_LOSS_LIMIT_USD, EQUITY_DRAWDOWN_CIRCUIT_BREAKER_PCT, } from './types.js';
...
export function getPortfolioRiskSnapshot(engine) {
...
export function evaluateSessionKpiGate(engine, symbol) {
    ...
    const entries = engine.getMetaJournalEntries()
        ...
        if (tagBucket.length > 0)
            return tagBucket === sessionBucket;
...
function flattenAllPositions(engine, reason) {
    for (const agent of engine.agents.values()) {
        if (!agent.position)
            continue;
        const symbol = engine.market.get(agent.config.symbol);
...
export function evaluatePortfolioCircuitBreaker(engine) {
    ...
    if (engine.circuitBreakerLatched && engine.circuitBreakerArmedAt) {
        ...
        const isRecovered = (engine.circuitBreakerScope === 'daily' && dayLossPct < AUTO_UNLATCH_RECOVERY_PCT) ||
...
export function evaluateCryptoExecutionGuard(engine, symbol, intel) {
