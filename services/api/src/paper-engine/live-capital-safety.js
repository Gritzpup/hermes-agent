...
const liveRoundTripPnl = [];
let cumulativeLivePnl = 0;
let liveRollbackUntil = null;
let liveRollbackReason = null;
export function recordLiveRoundTrip(pnl) {
    liveRoundTripPnl.push(pnl);
    if (liveRoundTripPnl.length > 20)
        liveRoundTripPnl.shift();
...
export function isLiveRollbackActive() {
...
export function getRollbackSnapshot() {
    ...
    if (last3.length === 3 && last3.every((p) => p < 0))
        consecutiveLosses = 3;
    else if (last3.length === 2 && last3.every((p) => p < 0))
        consecutiveLosses = 2;
    else if (last3.length === 1 && last3[0] !== undefined && last3[0] < 0)
        consecutiveLosses = 1;
    return {
...
export function clearLiveRollback() {
...
let _instance = null;
export function getLiveCapitalSafety() {
    if (!_instance)
        _instance = new LiveCapitalSafety();
    return _instance;
}
export class LiveCapitalSafety {
    // ── Constants (all env-overridable) ────────────────────────────────
    LIVE_MAX_NOTIONAL_USD = Number(process.env.LIVE_MAX_NOTIONAL_USD ?? 10);
    LIVE_MAX_CONCURRENT_POSITIONS = Number(process.env.LIVE_MAX_CONCURRENT_POSITIONS ?? 1);
    LIVE_MAX_TRADES_PER_DAY = Number(process.env.LIVE_MAX_TRADES_PER_DAY ?? 20);
    LIVE_MAX_SINGLE_LOSS_USD = Number(process.env.LIVE_MAX_SINGLE_LOSS_USD ?? 3);
    LIVE_MAX_TOTAL_DRAWDOWN_USD = Number(process.env.LIVE_MAX_TOTAL_DRAWDOWN_USD ?? 20);
    LIVE_PAPER_DIVERGENCE_PCT = Number(process.env.LIVE_PAPER_DIVERGENCE_PCT ?? 20);
    LIVE_DIVERGENCE_MIN_TRADES = Number(process.env.LIVE_DIVERGENCE_MIN_TRADES ?? 5);
    LIVE_HALT_EMBARGO_HOURS = Number(process.env.LIVE_HALT_EMBARGO_HOURS ?? 24);
    // ── State ───────────────────────────────────────────────────────────
    haltedUntil = 0;
    haltReason = '';
    ...
    recentDivergenceDeltas = []; // last 10 live-vs-paper pct deltas
    ...
    canOpenLivePosition(symbol, notional, currentConcurrentCount) {
    ...
    checkDivergence() {
        ...
        if (this.liveTradeStats.count >= this.LIVE_DIVERGENCE_MIN_TRADES &&
            this.recentDivergenceDeltas.length >= this.LIVE_DIVERGENCE_MIN_TRADES) {
            const avgDivergence = this.recentDivergenceDeltas.reduce((a, b) => a + b, 0) / this.recentDivergenceDeltas.length;
    ...
    triggerHalt(reason, durationHours = this.LIVE_HALT_EMBARGO_HOURS) {
    ...
    getSnapshot() {
        ...
        if (this.recentDivergenceDeltas.length >= this.LIVE_DIVERGENCE_MIN_TRADES) {
            divergencePct =
