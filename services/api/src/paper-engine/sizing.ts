...
import type { AgentState, AgentStyle } from './types.js';
 ...
 */
export function computeHalfKelly(recentOutcomes: number[]): number {
...
export function countConsecutiveLosses(outcomes: number[]): number {
  let count = 0;
  for (let index = outcomes.length - 1; index >= 0; index -= 1) {
    if ((outcomes[index] ?? 0) < 0) count += 1;
    else break;
  }
  return count;
}
...
export function relativeMove(history: number[], lookback: number): number {
 ...
 */
export function computeAdaptiveCooldown(
  baseCooldown: number,
  lastExitPnl: number,
 ...
 */
export function computeFngSizeMultiplier(
  style: AgentStyle,
  assetClass: string,
  fearGreedValue: number | null
): number {
  if (fearGreedValue === null || assetClass !== 'crypto') return 1.0;
...
export function computeStreakMultiplier(
  ...
  if (disableStreakPenalty) return 1.0;
