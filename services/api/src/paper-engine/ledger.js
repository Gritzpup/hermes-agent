...
export function enqueueWrite(filePath, operation) {
    const queue = fileQueues.get(filePath) ?? Promise.resolve();
    fileQueues.set(filePath, queue.then(async () => {
        try {
            await operation();
        }
        catch (error) {
...
export function appendLedger(filePath, payload) {
...
export function rewriteLedger(filePath, entries) {
    enqueueWrite(filePath, async () => {
...
export function maybeRotateLog(filePath, maxMB) {
    enqueueWrite(filePath, async () => {
        try {
