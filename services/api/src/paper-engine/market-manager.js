...
export class MarketManager {
    ...
    applySnapshot(symbol, snapshot, recordHistory) {
        ...
        if (recordHistory && nextPrice > 0) {
            ...
            if (symbol.history.length > 200)
                symbol.history.shift();
            symbol.returns.push(nextReturn);
            if (symbol.returns.length > 200)
                symbol.returns.shift();
        }
