...
export const REGIME_LANE_MULTIPLIERS: Record<string, Record<string, number>> = {
...
export interface CapitalManagerDeps {
  ...
  /** Returns the current regime string (e.g. 'compression', 'trending', 'panic', 'expansion', 'unknown'). */
  getRegime: () => string;
}
...
export class CapitalManager {
   ...
   */
  getDeskEquity(startingEquity = 300_000): number {
    ...
    if (brokerTotal > 0) return round(brokerTotal, 2);
   ...
   */
  getDeskStartingEquity(startingEquity = 300_000): number {
  ...
  refreshAllocation(): void {
    ...
    const rawScores = contenders.map((agent) => {
      ...
      const stableBias =
        agentAssetClass === 'crypto' && stableRegime?.regime === 'outflow' ? -0.10 :
      ...
      const score = clamp(
        0.35 + posteriorMean * 0.4 + clamp(profitFactor / 4, 0, 0.35)
          + clamp(expectancy / 40, -0.08, 0.08) + tapeBonus + convictionBonus + embargoPenalty + newsPenalty + liquidityBonus + stableBias,
        0.1, 1.8
      );
      ...
      const rawLane: string = agent.config.id.startsWith('maker-')
        ...
        : agent.config.id.startsWith('grid-')
          ...
          : agent.config.id.startsWith('pairs-')
            ? 'pairs'
            : 'scalping';
      const currentRegime: string = this.deps.getRegime();
