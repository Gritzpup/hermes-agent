 ...
 */
export function drawdownSizingFactor(engine) {
...
function getVenueMedianLatencyMs(report, venue) {
...
function isOptimalTradingSession(symbol, hour) {
...
function getSessionQuality(symbol, hour) {
...
export { getMetaLabelDecision, getContextualMetaSignal, buildEntryMeta, buildMetaCandidate, buildJournalContext } from './engine-entry-meta.js';
export { getRouteBlock, getPrecisionBlock, getManagerBlock, describeWatchState, describeAiState, normalizeFlowBucket, getConfidenceBucket, getSpreadBucket, entryNote, getEntryScore, entryThreshold, exitThreshold, estimatedBrokerRoundTripCostBps, fastPathThreshold, brokerRulesFastPathThreshold, canUseBrokerRulesFastPath } from './engine-entry-scoring.js';
...
export function canEnter(engine, agent, symbol, shortReturn, mediumReturn, score) {
    ...
    if (fs.existsSync(EMERGENCY_HALT_FILE)) {
        ...
        const haltLanes = haltData.lanes ?? null;
        const haltSymbols = haltData.symbols ?? null;
    ...
    if (fs.existsSync(SCALPING_DISABLED_FILE)) {
        ...
        if (lane === 'scalping') {
            ...
            const disabledSymbols = disabledData.symbols ?? null;
    ...
    if (agent.config.executionMode === 'broker-paper' && symbol.price > 0 && symbol.tradable) {
        if (engine.circuitBreakerLatched)
            return false;
        if (engine.operationalKillSwitchUntilMs > Date.now())
            return false;
        // COO FIX #1: Block entry if symbol hit daily loss limit
        if (agent.symbolKillSwitchUntil && new Date(agent.symbolKillSwitchUntil) > new Date())
            return false;
        ...
        if (symbol.spreadBps > agent.config.spreadLimitBps)
            return false;
        ...
        if (Math.abs(score) < engine.entryThreshold(agent.config.style) * cryptoThresholdMult)
            return false;
        ...
        if (!strongDirectionSignal && intel.confidence < scalperConfidenceFloor)
            return false;
        ...
        if (symbol.assetClass === 'crypto') {
            ...
            if (direction === 'long' && (riskOffActive || (panicRegime && negativeDrift)))
                return false;
        }
        ...
        if (agent.config.style === 'momentum' && direction === 'long' && (intel.direction === 'sell' || intel.direction === 'strong-sell'))
            return false;
        if (agent.config.style === 'momentum' && direction === 'short' && (intel.direction === 'buy' || intel.direction === 'strong-buy'))
            return false;
        // Fix #11: Block crowded trades — if Binance funding shows everyone positioned same way, skip
        if (symbol.assetClass === 'crypto' && engine.derivativesIntel.shouldBlockEntry(symbol.symbol, direction))
            return false;
        ...
        const recentSameClass = Array.from(engine.agents.values()).some((other) => {
            if (other.config.id === agent.config.id || !other.position)
                return false;
            if ((engine.tick - other.position.entryTick) >= 3)
                return false;
        ...
        if (openSameClass >= maxConcurrent)
            return false;
        if (engine.breachesCrowdingLimit(symbol))
            return false;
        // 4. Minimum price history: need at least 20 data points for indicators to work
        if (symbol.history.length < 20)
            return false;
        ...
        if (rsi2 !== null) {
            ...
            if (agent.config.style === 'mean-reversion' && direction === 'long' && rsi2 > rsi2LimitLong)
                return false;
            if (agent.config.style === 'mean-reversion' && direction === 'short' && rsi2 < rsi2LimitShort)
                return false;
            if (agent.config.style === 'momentum' && direction === 'long' && rsi2 > 85)
                return false;
            if (agent.config.style === 'momentum' && direction === 'short' && rsi2 < 18)
                return false;
        ...
        if (symbol.assetClass === 'forex' && agent.config.style === 'momentum') {
            ...
            if (stoch && stoch.crossover === 'bearish')
                return false;
        }
        ...
        if (symbol.assetClass === 'forex' && agent.config.style === 'mean-reversion') {
            const stoch = engine.marketIntel.computeStochastic(symbol.symbol);
            if (stoch && stoch.k > 50 && stoch.crossover !== 'bullish')
                return false;
        }
        ...
        if (rsi14 !== null) {
            ...
            if (agent.config.style === 'momentum' && direction === 'long' && rsi14 < 45)
                return false;
            ...
            if (agent.config.style === 'mean-reversion' && direction === 'long' && rsi14 > rsi14Limit)
                return false;
            // Short entries: momentum short needs RSI(14) < 55, mean-reversion short needs RSI(14) > 40
            if (agent.config.style === 'momentum' && direction === 'short' && rsi14 > 55)
                return false;
            if (agent.config.style === 'mean-reversion' && direction === 'short' && rsi14 < 40)
                return false;
        }
        // 10-14. Advanced crypto filters (from entry-filters module)
        if (symbol.assetClass === 'crypto') {
            const trend5m = engine.marketIntel.getTrend5m(symbol.symbol);
            if (trend5m === 'down' && direction === 'long' && agent.config.style === 'momentum')
                return false;
            if (trend5m === 'up' && direction === 'short' && agent.config.style === 'momentum')
                return false;
            if (agent.config.style !== 'mean-reversion' && engine.marketIntel.isLiquiditySweep(symbol.symbol))
                return false;
        ...
        const minNetEdgeBps = regime === 'panic'
            ...
            : regime === 'trend'
                ? 6
                : 4;
        if (meta.expectedNetEdgeBps < minNetEdgeBps)
            return false;
        ...
        if (proposedNotional > 0 && engine.wouldBreachPortfolioRiskBudget(agent, symbol, proposedNotional))
            return false;
        return true;
    }
    ...
    if (style === 'momentum') {
        if (isBrokerPaperEquity) {
            return (score > engine.entryThreshold(style)
                && shortReturn > 0.0005
                && shortSma > longSma
                && symbol.bias > 0
        ...
        if (isBrokerPaperCrypto) {
            return (score > engine.entryThreshold(style) * 1.4 // crypto selectivity boost
                && shortReturn > 0.0004
                && shortSma > longSma
                && symbol.bias > 0
                && symbol.spreadBps <= agent.config.spreadLimitBps);
        ...
        if (isBrokerPaperFx) {
            return (score > Math.max(engine.entryThreshold(style) - 0.5, 0.8)
                && shortReturn > 0.0002
    ...
    if (style === 'breakout') {
        ...
        if (isBrokerPaperEquity) {
            return (score > Math.max(engine.entryThreshold(style) + 1.1, 5.9)
                && shortReturn > 0.0019
                && mediumReturn > 0.0021
        ...
        if (isBrokerPaperFx) {
            return (score > Math.max(engine.entryThreshold(style) - 1.5, 3.0)
                && shortReturn > 0.0004
    ...
    if (isBrokerPaperCrypto) {
        return (score > Math.max(engine.entryThreshold(style) * 1.4 + 0.2, 1.45 * 1.4) // crypto selectivity boost
            && shortReturn < -0.0012
            && mediumReturn > -0.003
    ...
    if (isBrokerPaperFx) {
        return (score > Math.max(engine.entryThreshold(style) - 0.2, 1.0)
            && shortReturn < -0.0003
            && mediumReturn > -0.002
...
export function refreshScalpRoutePlan(engine) {
    ...
    for (const agent of engine.agents.values()) {
        ...
        const direction = meta.expectedNetEdgeBps > 0
            ? 'buy'
...
if (import.meta.url?.endsWith(process.argv[1]?.replace(/^file:\/\//, '') ?? '')) {
    ...
    const result = canEnter(mockEngine, mockAgent, mockSymbol, 0.001, 0.001, 1);
    ...
    const result2 = canEnter(mockEngine, momentumAgent, mockSymbol, 0.001, 0.001, 1);
    if (result2 !== false) {
