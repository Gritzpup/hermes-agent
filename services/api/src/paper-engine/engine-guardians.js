...
export function getSymbolCluster(engine, symbol) {
    if (symbol.assetClass === 'commodity' || symbol.assetClass === 'commodity-proxy')
        return 'commodity';
    if (symbol.assetClass === 'crypto')
        return 'crypto';
    if (symbol.assetClass === 'equity')
        return 'equity';
    if (symbol.assetClass === 'bond')
        return 'bond';
    return 'forex';
}
export function getClusterLimitPct(engine, cluster) {
    if (cluster === 'crypto')
        return 45;
    if (cluster === 'equity')
        return 35;
...
export function getSymbolGuard(engine, symbol) {
...
export function restoreSymbolGuards(engine) {
...
export function checkSymbolKillswitch(engine, agent) {
...
export function persistSymbolGuards(engine) {
    const path = SYMBOL_GUARD_PATH;
    try {
...
export function updateSymbolGuard(engine, symbol, mutation) {
...
export function applySpreadShockGuard(engine, symbol) {
    if (symbol.baseSpreadBps <= 0)
        return;
    const spreadShockRatio = symbol.spreadBps / symbol.baseSpreadBps;
    if (spreadShockRatio < 2.4)
        return;
    engine.updateSymbolGuard(symbol.symbol, (state) => ({
        ...state,
...
export function queueEventDrivenExit(engine, symbol, trigger) {
    for (const agent of engine.agents.values()) {
        if (!agent.position || agent.config.symbol !== symbol.symbol || agent.pendingOrderId)
            continue;
...
export async function processEventDrivenExitQueue(engine) {
    if (engine.pendingEventExitReasons.size === 0)
        return;
...
export function wouldBreachPortfolioRiskBudget(engine, agent, symbol, proposedNotional) {
    const snapshot = engine.getPortfolioRiskSnapshot();
    const cluster = engine.getSymbolCluster(symbol);
    const clusterInfo = snapshot.byCluster.find((c) => c.cluster === cluster);
    if (!clusterInfo)
        return false;
...
export function evaluateSessionKpiGate(engine, symbol) {
...
export function buildRegimeKpis(engine, regime, entries) {
    const filtered = entries.filter(e => e.regime === regime || e.context?.regime === regime);
    if (filtered.length === 0)
        return { samples: 0, winRate: 0, profitFactor: 1 };
...
export function getRegimeThrottleMultiplier(engine, regime, style) {
    if (regime === 'panic')
        return 0.2; // 80% throttle in panic
    if (regime === 'chop' && style === 'momentum')
        return 0.5;
    if (regime === 'trend' && style === 'mean-reversion')
        return 0.6;
    return 1.0;
}
export function computeConfidenceCalibrationMultiplier(engine, agent, decision) {
