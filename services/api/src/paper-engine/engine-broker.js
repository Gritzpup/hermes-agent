...
import { PAPER_BROKER, BROKER_ROUTER_URL, FILL_LEDGER_PATH, BROKER_SYNC_MS, TICK_MS, } from './types.js';
// Re-export execution functions so paper-engine.ts imports don't break
export { routeBrokerOrder, openBrokerPaperPosition, closeBrokerPaperPosition, applyBrokerFilledEntry, applyBrokerFilledExit, fetchBrokerAccount, finalizeBrokerFlat, handleAsyncOrderStatus, recentlyExpiredOrderIds, recordExpiredOrder } from './engine-broker-execution.js';
import { fetchBrokerAccount } from './engine-broker-execution.js';
export async function seedFromBrokerHistory(engine) {
    try {
        ...
        for (const broker of brokers) {
            ...
            if (broker.broker === 'alpaca-paper') {
                ...
                const alpacaAgents = Array.from(engine.agents.values()).filter((a) => a.config.broker === 'alpaca-paper');
                const alpacaTrades = alpacaAgents.reduce((s, a) => s + a.trades, 0);
                if (alpacaTrades > 0)
                    continue;
                const orders = Array.isArray(broker.orders) ? broker.orders : [];
                const fills = orders.filter((o) => o.status === 'filled' && typeof o.filled_avg_price === 'string');
            ...
            if (broker.broker === 'oanda-rest') {
                const oandaAgents = Array.from(engine.agents.values()).filter((a) => a.config.broker === 'oanda-rest');
                const oandaTrades = oandaAgents.reduce((s, a) => s + a.trades, 0);
                if (oandaTrades > 0)
                    continue;
                const acct = broker.account ?? {};
                const oandaPl = parseFloat(String(acct.pl ?? '0'));
                const oandaFills = Array.isArray(broker.fills) ? broker.fills : [];
                // Count unique instruments that have been traded (open trades = evidence of activity)
                const tradedInstruments = new Set(oandaFills.map((f) => String(f.instrument ?? '')).filter(Boolean));
                if (oandaPl !== 0 && tradedInstruments.size > 0) {
                    ...
                    for (const instrument of tradedInstruments) {
                        ...
                        const instrumentFills = oandaFills.filter((f) => f.instrument === instrument);
                        agent.trades = instrumentFills.length;
                        agent.realizedPnl = round(perInstrument, 4);
                        if (perInstrument >= 0)
                            agent.wins = Math.max(1, Math.round(instrumentFills.length * 0.6));
                        else
                            agent.losses = instrumentFills.length;
                    }
                }
            }
        }
        ...
        const totalSeeded = Array.from(engine.agents.values()).reduce((s, a) => s + a.trades, 0);
        const totalPnl = Array.from(engine.agents.values()).reduce((s, a) => s + a.realizedPnl, 0);
        if (totalSeeded > 0) {
...
export async function reconcileBrokerPaperState(engine, force = false) {
    const now = Date.now();
...
function isNearSessionEndUtc() {
    ...
    if (utcHour === nyCloseHour - 1 && utcMin >= 60 - OANDA_FLATTEN_BEFORE_MINUTES)
        return true;
    return false;
}
export function flattenOandaBeforeSessionEnd(engine) {
    if (!isNearSessionEndUtc())
        return;
    for (const agent of engine.agents.values()) {
...
export function syncBrokerPositionIntoAgent(engine, agent, symbol, brokerPosition, openedAt) {
