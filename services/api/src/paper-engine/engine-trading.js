...
export { manageOpenPosition, openPosition, closePosition, updateArbAgent } from './engine-trading-positions.js';
export { getPositionDirection, getPositionUnrealizedPnl, maybeTrailBrokerStop, getFeeRate, roundTripFeeBps, computeDynamicStop, computeDynamicTarget, resolveEntryDirection, computeGrossPnl, getSessionBucket, getVolatilityBucket, noteTradeOutcome, getAgentNetPnl, getAgentEquity, getDeskEquity, getBenchmarkEquity, computeHalfKelly, breachesCrowdingLimit, getEffectiveLeverage } from './engine-trading-helpers.js';
...
export async function step(engine, isRedisTick = false) {
...
export async function updateAgent(engine, agent) {
    const symbol = engine.market.get(agent.config.symbol);
    if (!symbol)
        return;
    ...
    const intel = engine.marketIntel.getCompositeSignal(symbol.symbol);
    const intelBlocked = intel.tradeable && ((score > 0 && (intel.direction === 'sell' || intel.direction === 'strong-sell')) ||
