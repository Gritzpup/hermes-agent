 ...
 */
import { computeRSI, computeRSI2 as _computeRSI2, computeRSI14 as _computeRSI14, computeRSI14_5m as _computeRSI14_5m, computeMACD, computeMACD15m as _computeMACD15m, computeBollinger as _computeBollinger, computeVwap as _computeVwap, computeATR as _computeATR, computeStochastic as _computeStochastic, computeWeightedOBI as _computeWeightedOBI, ema as _ema, } from './market-intel-indicators.js';
import { computeCompositeSignal, getSupportResistance as _getSupportResistance, } from './market-intel-signals.js';
...
const CMC_SYMBOLS = ['BTC', 'ETH', 'SOL', 'XRP'];
const CMC_TO_OUR_SYMBOL = {
    BTC: 'BTC-USD', ETH: 'ETH-USD', SOL: 'SOL-USD', XRP: 'XRP-USD',
};
...
export class MarketIntelligence {
    ...
    barHistory = new Map(); // 60s bars
    currentBar = new Map();
    bar5mHistory = new Map(); // 5-minute bars
    currentBar5m = new Map();
    bar15mHistory = new Map(); // 15-minute bars
    currentBar15m = new Map();
    symbols;
    timer = null;
    fngTimer = null;
    fundingTimer = null;
    ...
    fearGreedPollInFlight = false;
    fundingPollInFlight = false;
    cmcPollInFlight = false;
    fundingRates = new Map();
    cmcData = new Map();
    hlTimer = null;
    hlPollInFlight = false;
    hlData = new Map();
    hlPrevOi = new Map();
    ivTimer = null;
    ivPollInFlight = false;
    ivData = new Map();
    stableTimer = null;
    stablePollInFlight = false;
    stableSignal = null;
    stablePrevTotal = 0;
    stablePrevTotalAt = 0;
    // Blockchain.info BTC chain health (Phase 3.2)
    btcChainTimer = null;
    btcChainPollInFlight = false;
    btcChainHealth = { hashrateEhs: null, mempoolFeesSatPerVB: null, regime: 'unknown' };
    prevHashrate = null;
    prevHashrateAt = null;
    ...
    oandaAlpacaTimer = null;
    oandaAlpacaPollInFlight = false;
    constructor(symbols) {
    ...
    stop() {
        ...
        if (this.btcChainTimer) {
            clearInterval(this.btcChainTimer);
            this.btcChainTimer = null;
        }
        if (this.oandaAlpacaTimer) {
            clearInterval(this.oandaAlpacaTimer);
            this.oandaAlpacaTimer = null;
        }
        // Phase 4.1 + 4.2 external pollers
        stopEtherscanWhales();
        stopApifyCongress();
    }
    feedOrderFlow(signal) {
    ...
    updateBar(symbol, price, volume, now, periodMs, currentMap, historyMap) {
    ...
    getSnapshot() {
        ...
        const bollinger = this.symbols.map((s) => this.computeBollinger(s)).filter((b) => b !== null);
        const vwap = this.symbols.map((s) => this.computeVwap(s)).filter((v) => v !== null);
    ...
    getRecentVolRatio(symbol, lookback = 5) {
        const bars = this.barHistory.get(symbol);
        if (!bars || bars.length < lookback + 14)
            return null;
    ...
    async pollOrderbooks() {
        if (this.orderbookPollInFlight)
            return;
        ...
        try {
            ...
            await Promise.all(symbols.map(async (symbol) => {
                try {
                    ...
                    const direction = absImb < 15 ? 'neutral' : imbalancePct > 0 ? 'buy' : 'sell';
                    const strength = absImb > 60 ? 'strong' : absImb > 30 ? 'moderate' : 'weak';
    ...
    async pollFearGreed() {
        ...
        try {
            ...
            const regime = value <= 20 ? 'extreme-fear' :
                value <= 40 ? 'fear' :
                    value <= 60 ? 'neutral' :
                        value <= 80 ? 'greed' : 'extreme-greed';
            const contrarian = value <= 20 ? 'buy' : value >= 80 ? 'sell' : 'neutral';
            this.fearGreed = {
                value,
        ...
        try {
            ...
            await Promise.all(entries.map(async ([ourSymbol, okxSymbol]) => {
                try {
                    ...
                    const bias = annualizedPct >= 20 ? 'sell' :
    ...
    getAllFundingDivergences() {
        const both = new Set([...this.fundingRates.keys()].filter((s) => this.hlData.has(s)));
        return [...both].map((s) => this.getFundingDivergence(s)).filter((d) => d !== null);
    }
    getCmcSignal(symbol) {
        return this.cmcData.get(symbol) ?? null;
    }
    getHyperliquidSignal(symbol) {
        return this.hlData.get(symbol) ?? null;
    }
    getImpliedVolSignal(symbol) {
    ...
    getCongressionalBias(symbol) {
        ...
        for (const t of relevant) {
            if (t.type === 'purchase' && t.valueUsd >= 50_000)
                score += 5;
            else if (t.type === 'sale' && t.valueUsd >= 50_000)
                score -= 5;
        }
    ...
    getLastPrice(symbol) {
        if (symbol === 'BTC-USD')
            return this._btcLastPrice;
        return this.priceHistory.get(symbol)?.at(-1) ?? null;
    }
    async pollCmc() {
        if (this.cmcPollInFlight)
            return;
    ...
    async pollImpliedVol() {
        if (this.ivPollInFlight)
            return;
        ...
        try {
            ...
            await Promise.all(Object.entries(DVOL_SYMBOL_MAP).map(async ([ourSymbol, currency]) => {
                try {
                    ...
                    if (typeof dvol !== 'number' || !Number.isFinite(dvol))
                        return;
                    const regime = dvol < 35 ? 'crushed' :
                        dvol < 60 ? 'normal' :
    ...
    async pollStablecoins() {
        ...
        try {
            ...
            const regime = changePct24h === null ? 'neutral' :
    ...
    async pollBtcChainHealth() {
        if (this.btcChainPollInFlight)
            return;
        ...
        try {
            ...
            let mempoolFeesSatPerVB = null;
            if (feeResp.ok) {
                const text = await feeResp.text();
                const v = Number(text);
                if (Number.isFinite(v) && v > 0)
                    mempoolFeesSatPerVB = v;
            }
            ...
            if (hashrateEhs !== null || mempoolFeesSatPerVB !== null) {
                ...
                if (mempoolFeesSatPerVB !== null && mempoolFeesSatPerVB > 100)
                    stressed = true;
     ...
     */
    async pollOandaAlpaca() {
        if (this.oandaAlpacaPollInFlight)
            return;
    ...
    computeBollinger(symbol) {
        return _computeBollinger(this.priceHistory, symbol);
    }
    computeVwap(symbol) {
        return _computeVwap(this.volumeHistory, symbol);
    }
    computeComposite(symbol) {
        return computeCompositeSignal(symbol, this.orderFlow, this.fearGreed, this.priceHistory, this.volumeHistory, this.barHistory, this.fundingRates, this.hlData, symbol === 'BTC-USD' ? this._venueDivergence : false, this.stableSignal);
    }
    /** Called by venue-sanity.ts to cache the latest divergence flag. */
    setVenueDivergence(divergent) {
        this._venueDivergence = divergent;
    }
    computeATR(symbol, period = 14) {
     ...
     */
    computeStochastic(symbol, kPeriod = 14, dPeriod = 3) {
