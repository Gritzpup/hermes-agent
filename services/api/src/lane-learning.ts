...
import type { StrategyMode, TradeJournalEntry } from '@hermes/contracts';
...
function normalizeEntry(entry: TradeJournalEntry): TradeJournalEntry {
...
export class LaneLearningEngine {
  private readonly lastSerializedDecision = new Map<string, string>();

  review(entries: TradeJournalEntry[]): LaneLearningDecision[] {
    const normalized = entries.map(normalizeEntry)
      .filter((entry): entry is TradeJournalEntry & { strategyId: string; lane: Extract<StrategyMode, 'pairs' | 'grid' | 'maker'> } =>
        entry.strategyId !== undefined && (entry.lane === 'pairs' || entry.lane === 'grid' || entry.lane === 'maker'));
    const groups = new Map<string, Array<TradeJournalEntry & { strategyId: string; lane: Extract<StrategyMode, 'pairs' | 'grid' | 'maker'> }>>();
    ...
    const decisions: LaneLearningDecision[] = [];
  ...
  getLog(limit = 100): LaneLearningDecision[] {
    try {
      ...
      return fs.readFileSync(LOG_PATH, 'utf8')
        ...
        .map((line) => JSON.parse(line) as LaneLearningDecision)
        .slice(-limit);
    } catch {
      return [];
    }
  ...
  private maybeLog(decision: LaneLearningDecision): void {
