...
function inferVenueFromSource(source, agentId) {
    if (source === 'broker') {
        if (agentId.includes('coinbase') || agentId.includes('cb-'))
            return 'coinbase';
        if (agentId.includes('alpaca') || agentId.includes('paper'))
            return 'alpaca';
        if (agentId.includes('oanda'))
            return 'oanda';
        return 'unknown';
    }
    return 'simulated';
}
function inferLiquidity(lane) {
    if (!lane)
        return 'taker';
    const l = lane.toLowerCase();
    if (l.includes('maker') || l.includes('mm'))
        return 'maker';
    return 'taker';
}
...
// ── Core Logic ─────────────────────────────────────────────────────────────────
function readFills(lookbackDays = 7) {
...
function buildTradePairs(fills) {
    const pairs = [];
    const buys = new Map();
    const sells = new Map();
...
function computeRealizedFeeBps(pair) {
    if (pair.source !== 'broker')
        return null;
    if (pair.pnlImpact === 0)
        return null; // skip zero-PnL trades
...
function getEstimatedFeeBps(symbol, liquidity) {
    const assetClass = inferAssetClassFromSymbol(symbol);
    const broker = symbol.includes('_') ? 'oanda-rest' : 'coinbase-live';
...
export function reconcileFees(lookbackDays = 7) {
    ...
    const realizedByBucket = new Map();
    ...
    for (const [key, fees] of realizedByBucket) {
        ...
        if (flagReasonStr)
            bucket.flagReason = flagReasonStr;
        buckets.push(bucket);
        if (flagged) {
            const confidence = fees.length >= 30 ? 'high' : fees.length >= 10 ? 'medium' : 'low';
            warnings.push({
                timestamp: now,
                bucket,
...
export function getLatestReport() {
    ...
    for (const line of lines) {
        try {
            ...
            warnings.push(w);
            if (w.timestamp > latestTimestamp)
...
// ── Startup Hook ──────────────────────────────────────────────────────────────
export function runFeeReconciliationOnStartup() {
