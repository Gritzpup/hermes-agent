...
// ─── RSI ─────────────────────────────────────────────────────────────────────
export function computeRSI(prices, period) {
...
export function computeRSI2(priceHistory, symbol) {
    return computeRSI(priceHistory.get(symbol) ?? [], 2);
}
/** RSI(14) — standard period RSI */
export function computeRSI14(priceHistory, symbol) {
    return computeRSI(priceHistory.get(symbol) ?? [], 14);
}
/** RSI(14) on 5-minute bars */
export function computeRSI14_5m(bar5mHistory, symbol) {
...
export function computeMACD15m(bar15mHistory, symbol) {
    const bars = bar15mHistory.get(symbol);
    if (!bars || bars.length < 27)
        return null;
    const macd = computeMACD(bars.map((b) => b.close));
    return macd?.histogram ?? null;
}
// ─── Bollinger Bands ─────────────────────────────────────────────────────────
export function computeBollinger(priceHistory, symbol) {
...
// ─── VWAP ────────────────────────────────────────────────────────────────────
export function computeVwap(volumeHistory, symbol) {
    ...
    const signal = deviation < -0.1 ? 'buy' : deviation > 0.1 ? 'sell' : 'neutral';
    ...
    if (vh.length >= 20) {
        ...
        for (const p of recentHalf) {
            recentPV += p.price * p.volume;
            recentV += p.volume;
        }
        for (const p of olderHalf) {
            olderPV += p.price * p.volume;
            olderV += p.volume;
        }
...
// ─── ATR ─────────────────────────────────────────────────────────────────────
export function computeATR(barHistory, priceHistory, symbol, period = 14) {
...
// ─── Stochastic ──────────────────────────────────────────────────────────────
export function computeStochastic(barHistory, priceHistory, symbol, kPeriod = 14, dPeriod = 3) {
    const bars = barHistory.get(symbol);
    const useBars = bars && bars.length >= kPeriod + dPeriod;
    const prices = priceHistory.get(symbol);
    if (!useBars && (!prices || prices.length < kPeriod + dPeriod))
        return null;
    const kValues = [];
    const dataLen = useBars ? bars.length : prices.length;
    for (let end = dataLen - dPeriod - 1; end < dataLen; end++) {
        let lowestLow, highestHigh, close;
        if (useBars) {
...
// ─── Weighted OBI ────────────────────────────────────────────────────────────
export function computeWeightedOBI(orderFlow, symbol) {
