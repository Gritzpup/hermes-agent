...
let lastTrades: CongressionalTrade[] = [];
...
async function discoverActor(): Promise<string | null> {
  ...
  try {
    ...
    const found = items.find((a) =>
      a.name.toLowerCase().includes('congress') ||
...
async function pollOnce(): Promise<void> {
  ...
  if (CONGRESS_ACTOR_ID === '' && resolvedActorId === null) {
    ...
    if (resolvedActorId) CONGRESS_ACTOR_ID = resolvedActorId;
  }
  ...
  try {
    ...
    const runResp = await fetchWithTimeout(
      `https://api.apify.com/v2/acts/${CONGRESS_ACTOR_ID}/runs/last/dataset/items?token=${token}&clean=1`,
    );
    if (!runResp.ok) return;

    const items = await runResp.json() as Array<{
      symbol?: string;
      ticker?: string;
      member?: string;
...
export function startApifyCongress(): void {
  if (pollTimer) return;
  pollTimer = setInterval(() => { void pollOnce(); }, POLL_MS);
  void pollOnce();
}

export function stopApifyCongress(): void {
 ...
 */
export function getCongressionalTrades(): CongressionalTrade[] {
  return lastTrades;
}
