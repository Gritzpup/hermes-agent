...
import type { BacktestAgentConfig, BacktestCandle, BacktestFill } from '@hermes/contracts';
...
export interface WFFoldWindow {
  /** 0-indexed fold number */
  foldIndex: number;
  /** ISO timestamp: train window start */
  trainStart: string;
  /** ISO timestamp: train window end — purge buffer ends here */
  trainEnd: string;
  /** ISO timestamp: validation window start (immediately after trainEnd) */
  valStart: string;
  /** ISO timestamp: validation window end */
  valEnd: string;
  /** Purge buffer applied to trainEnd: trainEnd - purgeBuffer */
  purgeCutoff: string;
}

export interface WFPurgeResult {
  /** All fills within the train window, with purge-applied removals */
  purgedFills: BacktestFill[];
  /** Count of fills removed by purge */
  purgedCount: number;
  /** Total fills before purge */
  totalCount: number;
}
...
export interface WFFoldResult {
  ...
  valCandles: number;
  trainFills: number;
  purgedTrainFills: number;
  purgedCount: number;
  valFills: number;
  /** Expectancy in R (PnL / starting equity) */
  trainExpectancy: number;
  valExpectancy: number;
  valWinRate: number;
  valProfitFactor: number;
  valSharpeRatio: number;
  valMaxDrawdownPct: number;
  valTotalReturnPct: number;
  passed: boolean;
}

export interface WFPartitionResult {
  symbol: string;
  totalCandles: number;
  totalDateRangeMs: number;
  folds: WFFoldResult[];
  overallValExpectancy: number;
  overallValWinRate: number;
  overallValProfitFactor: number;
  foldsPassed: number;
  foldsTotal: number;
  configHash: string;
}
 ...
 */
export function buildFoldWindows(
  candles: BacktestCandle[],
  numFolds: number = WF_MAX_FOLDS
): WFFoldWindow[] {
  ...
  const foldWindows: WFFoldWindow[] = [];
 ...
 */
export function applyPurge(fills: BacktestFill[], purgeCutoff: string): WFPurgeResult {
 ...
 */
export function extractFillsInWindow(
  fills: BacktestFill[],
  windowStart: string,
  windowEnd: string
): BacktestFill[] {
 ...
 */
export function computeExpectancy(fills: BacktestFill[], startingEquity: number = 100_000): number {
  if (fills.length === 0 || startingEquity === 0) return 0;
 ...
 */
export function computeWinRate(fills: BacktestFill[]): number {
 ...
 */
export function computeProfitFactor(fills: BacktestFill[]): number {
 ...
 */
export function hashConfig(config: BacktestAgentConfig): string {
  const fields = [
    config.style,
    config.targetBps,
    config.stopBps,
...
export type RegimeLabel = 'panic' | 'trend' | 'normal' | 'compression' | 'chop' | 'unknown';

export interface RegimeClassifiedFill extends BacktestFill {
  regime: RegimeLabel;
}
 ...
 * @returns RegimeLabel
 */
export function classifyRegime(candles: BacktestCandle[]): RegimeLabel {
  ...
  if (avgDailyRange >= 6 || totalReturn >= 2.5) return 'panic';
  if (totalReturn >= 1.1) return 'trend';
  if (avgDailyRange <= 1.2 && totalReturn <= 0.4) return 'compression';
  return 'chop';
}
